<#
.TITLE
    Remediation - Restart Windows Update Service

.SYNOPSIS
    Restarts the Windows Update service and verifies that it returns to a running state.

.DESCRIPTION
    Paired remediation for Restart-WindowsUpdate. Runs only when
    detect-Restart-WindowsUpdate.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) Restart-Service fix with failure tracking, (3) post-remediation
...
.TAGS
    Remediation,Action,WindowsUpdate,Services
...
.PERMISSIONS
    None (local SYSTEM context) - restarts a local Windows service.
...
.EXAMPLE
    ...
    Restarts wuauserv and verifies the running state; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Restart-WindowsUpdate\Restart-WindowsUpdate-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The service must exist before Restart-Service can act on it.
        $service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
        if (-not $service) {
            throw "Required Windows service not found: $ServiceName"
        }
...
function Test-FixApplied {
    try {
        # Re-check the same state the detector evaluated. Return $true only when
        # the observed state now matches the compliant definition.
        $service = Get-Service -Name $ServiceName -ErrorAction Stop
        return ($service.Status -eq 'Running')
    }
    catch {
        Write-RemediationLog "Verification could not read service ${ServiceName}: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "Restart service $ServiceName" -Fix {
        Write-RemediationLog "Restarting Windows Update service: $ServiceName" -Level 'Info'
