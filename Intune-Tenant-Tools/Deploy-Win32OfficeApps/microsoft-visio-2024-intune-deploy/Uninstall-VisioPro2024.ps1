<#
.TITLE
    Uninstall-VisioPro2024

.SYNOPSIS
    Silent uninstaller for Microsoft Visio Professional 2024 (volume) via the Office Deployment Tool.

.DESCRIPTION
    PowerShell wrapper for `setup.exe /configure Uninstall-VisioPro2024.xml` designed to run under
...
.TAGS
    Intune,Win32App,Office,Visio,Deployment,Uninstall
...
.CHANGELOG
    1.2.0 (2026-09-16) - Migrated from VisioPro2019Volume to VisioPro2024Volume
                         (PerpetualVL2024 channel); XML renamed to Uninstall-VisioPro2024.xml;
                         logs to <SystemDrive>\IntuneLogs\VisioPro2024\.
    1.1.0 (2026-09-16) - Canonical rewrite: uninstall now removes VisioPro2019Volume (the product
                         this package installs) instead of VisioProRetail, adds canonical logging
                         to <SystemDrive>\IntuneLogs\VisioPro2024\, and forwards the real setup.exe
                         exit code to Intune.
...
.PARAMETER ConfigurationPath
    Path to Uninstall-VisioPro2024.xml. Defaults to the file beside this script.

.PARAMETER SetupPath
    Path to setup.exe (Office Deployment Tool). Defaults to the file beside this script.

.EXAMPLE
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\Uninstall-VisioPro2024.ps1
    Removes the volume-licensed Visio Professional 2024 product silently.
...
if (-not $ConfigurationPath) { $ConfigurationPath = Join-Path $scriptBase 'Uninstall-VisioPro2024.xml' }
...
Write-Log -Message "Removing Visio Professional 2024 with: $ConfigurationPath" -Level 'INFO'
...
try {
    ...
    if ($exitCode -eq 0) {
        Write-Log -Message 'Visio Professional 2024 removed successfully.' -Level 'SUCCESS'
    } else {
