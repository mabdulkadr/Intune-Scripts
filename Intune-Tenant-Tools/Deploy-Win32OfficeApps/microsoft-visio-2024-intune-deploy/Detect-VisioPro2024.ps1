...
.SYNOPSIS
    Intune custom detection script for Visio Professional 2024 (volume, Click-to-Run).

.DESCRIPTION
    Verifies that Visio Professional 2024 (volume licensed) is installed and that a core
    application (VISIO.EXE) exists on disk. Touches only the Click-to-Run registry hive and
...
.TAGS
    Intune,Detection,Office,Visio,ClickToRun
...
.CHANGELOG
    1.2.0 (2026-09-16) - Migrated to VisioPro2024Volume (PerpetualVL2024 channel); renamed
                         log folder to <SystemDrive>\IntuneLogs\VisioPro2024\.
    1.1.0 (2026-09-16) - Canonical rewrite replacing the third-party Remy Kuster template whose
                         ...
                         detected. Now reads the Click-to-Run registry (VisioPro2019Volume),
                         version presence, and VISIO.EXE on disk with the canonical 0/1/2 exit
...
.NOTES
    ...
    - Read-only by design: registry + file probes plus a run transcript - never repackages or
      reinstalls anything.
    - Logs: <SystemDrive>\IntuneLogs\VisioPro2024\VisioPro2024-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    if (([string]$config.ProductReleaseIds) -notmatch [regex]::Escape('VisioPro2024Volume')) {
        $reasons.Add("Product 'VisioPro2024Volume' not present in ProductReleaseIds: $($config.ProductReleaseIds)")
    }
    ...
    if ($installPath -and -not (Test-Path -LiteralPath (Join-Path $installPath 'root\Office16\VISIO.EXE'))) {
        $reasons.Add("VISIO.EXE not found under InstallPath '$installPath'.")
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message 'Visio Professional 2024 detected as installed' -Level 'SUCCESS'
    }
