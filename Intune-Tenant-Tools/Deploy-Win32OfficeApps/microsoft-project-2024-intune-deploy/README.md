...
# 🛡️ Microsoft Project Professional 2024 - Intune Win32 Deployment

**[Silent, volume-licensed deployment package for Microsoft Project Professional 2024 via Intune Win32 apps.]**

Packages the Office Deployment Tool with a silent 64-bit/32-bit Project volume configuration, PowerShell install/uninstall scripts with dedicated logging, and a custom Intune detection script for Enterprise enrollment.
...
**Microsoft Project Professional 2024 - Intune Win32 Deployment** deploys Project Professional 2024 (volume licensed, `ProjectPro2024Volume`) to Intune-managed Windows devices using the official Microsoft Office Deployment Tool.
...
* Dedicated transcript log per operation under `C:\IntuneLogs\ProjectPro2024\`.
* Propagates the real `setup.exe` exit code instead of swallowing it.

### 🔹 Intune-Ready Detection
* Registry-driven (`HKLM\SOFTWARE\Microsoft\Office\ClickToRun\Configuration`) with disk-level hardening (`WINPROJ.EXE`).
...
├── Detect-ProjectPro2024.ps1       Intune Win32 App custom detection script
└── README.md
```

> **Legacy file:** `Install_MS_project.intunewin` is the original `.intunewin` package from a prior build and was removed. This folder contains no `.intunewin` yet — package it fresh with [IntuneWin-Utility](https://github.com/mabdulkadr/IntuneWin-Utility) or `IntuneWinAppUtil.exe` before deployment.
...
**File**
```powershell
Detect-ProjectPro2024.ps1
```

### Purpose
Reads the authoritative Click-to-Run registry and reports whether Project Professional 2024 is genuinely present. Read-only apart from the run transcript; never modifies the system.

### Logic
1. Reads `HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration` (exit `2` if missing/failed).
2. Confirms `ProductReleaseIds` contains `ProjectPro2024Volume` (exit `1` if not).
3. Confirms `VersionToReport` is populated (exit `1` if empty — partial install).
4. Confirms `WINPROJ.EXE` exists under `InstallPath\root\Office16\` (exit `1` if missing).
...
### Uninstall-ProjectPro2024.xml
Removes `ProjectPro2024Volume` only — leaves other Office products untouched.
...
### Logging
* Install/Uninstall transcripts: `C:\IntuneLogs\ProjectPro2024\`.
...
```powershell
IntuneWinAppUtil.exe -c "C:\microsoft-project-2024-intune-deploy" -s setup.exe -o "C:\intune-output"
```

### App Fields
| Setting | Value |
| ------- | ----- |
| Name | Microsoft Project Professional 2024 (Volume) |
| Install command | `powershell.exe -ExecutionPolicy Bypass -NoProfile -File "Install-ProjectPro2024.ps1"` |
| Uninstall command | `powershell.exe -ExecutionPolicy Bypass -NoProfile -File "Uninstall-ProjectPro2024.ps1"` |
...
### Detection Rules
Upload `Detect-ProjectPro2024.ps1` as a **custom detection script**:
...
**Alternative (no script):** Registry rule → `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Office\ClickToRun\Configuration` → value `ProductReleaseIds` → **contains** `ProjectPro2024Volume`.
...
3. The Intune Management Extension downloads, extracts, and runs `Install-ProjectPro2024.ps1` as SYSTEM (elevation probe passes).
4. The install script reads the current Office architecture and selects the matching XML.
5. ODT executes `setup.exe /configure Install-ProjectPro2024-{arch}-{lang}.xml` silently.
6. On the next check-in, `Detect-ProjectPro2024.ps1` runs; exit `0` marks the app **Installed**.
...
| `WINPROJ.EXE` not found after install | Office was installed to a non-default path — the detection script reads `InstallPath` from the ClickToRun registry |

---

# ❓ FAQ

**Can I install Project alongside Office LTSC 2024?**
Yes — Project 2024 volume installs independently alongside other Office products. All products share the same Click-to-Run configuration, but each has its own `ProductReleaseIds` entry.

**Can I use this with Microsoft 365 Apps?**
No. This configuration is Project 2024-only (`ProjectPro2024Volume`, `PerpetualVL2024`). For Project Plan 5/3 (M365), generate a retail config via [config.office.com](https://config.office.com).
