<#
.TITLE
    Detect-ProjectPro2024

.SYNOPSIS
    Intune custom detection script for Project Professional 2024 (volume, Click-to-Run).

.DESCRIPTION
    Verifies that Project Professional 2024 (volume licensed) is installed and that a core
    application (WINPROJ.EXE) exists on disk. Touches only the Click-to-Run registry hive and
...
.TAGS
    Intune,Detection,Office,Project,ClickToRun
...
.CHANGELOG
    1.2.0 (2026-09-16) - Migrated to ProjectPro2024Volume (PerpetualVL2024 channel); renamed
                         log folder to <SystemDrive>\IntuneLogs\ProjectPro2024\.
    1.1.0 (2026-09-16) - Canonical rewrite replacing the third-party Remy Kuster template whose
                         "not installed" paths all exited 0, so the app was always reported as
                         detected. Now reads the Click-to-Run registry (ProjectPro2019Volume),
                         version presence, and WINPROJ.EXE on disk with the canonical 0/1/2 exit
                         contract and structured logging.
    1.0.0 (2023)        - Original detection.
...
.NOTES
    ...
    - Read-only by design: registry + file probes plus a run transcript - never repackages or
      reinstalls anything.
    - Logs: <SystemDrive>\IntuneLogs\ProjectPro2024\ProjectPro2024-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    if (([string]$config.ProductReleaseIds) -notmatch [regex]::Escape('ProjectPro2024Volume')) {
        $reasons.Add("Product 'ProjectPro2024Volume' not present in ProductReleaseIds: $($config.ProductReleaseIds)")
    }
    ...
    if ($installPath -and -not (Test-Path -LiteralPath (Join-Path $installPath 'root\Office16\WINPROJ.EXE'))) {
        $reasons.Add("WINPROJ.EXE not found under InstallPath '$installPath'.")
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message 'Project Professional 2024 detected as installed' -Level 'SUCCESS'
    }
