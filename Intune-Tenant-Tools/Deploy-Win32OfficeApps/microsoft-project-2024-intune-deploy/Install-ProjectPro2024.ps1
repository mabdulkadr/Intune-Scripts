<#
.TITLE
    Install-ProjectPro2024

.SYNOPSIS
    Silent installer for Microsoft Project Professional 2024 (volume) via the Office Deployment Tool.

.DESCRIPTION
    PowerShell wrapper for `setup.exe /configure Install-ProjectPro2024-<arch>.xml` designed to run
    ...
    - Detects the installed Office Click-to-Run architecture and selects the matching Project
      configuration (x64 / x86); overridable with -Architecture. Defaults to x64 when no Office
      is present.
    - Installs in the OS language (MatchOS) via Install-ProjectPro2024-<arch>.xml; one config per architecture.
...
.TAGS
    Intune,Win32App,Office,Project,Deployment,Install
...
.CHANGELOG
    1.5.0 (2026-09-17) - Consolidated to one config per architecture
                          (Install-ProjectPro2024-x64.xml / -x86.xml) with MatchOS
    ...
    1.2.0 (2026-09-16) - Migrated from ProjectPro2019Volume to ProjectPro2024Volume
                         ...
                         Install-ProjectPro2024-<arch>-<lang>.xml; logs to
                         <SystemDrive>\IntuneLogs\ProjectPro2024\.
    1.1.0 (2026-09-16) - Canonical rewrite: correct ProductReleaseIds (ProjectPro2019Volume),
                         auto-detect architecture, -Language parameter (Bilingual/English/Arabic),
                         and canonical logging to <SystemDrive>\IntuneLogs\ProjectPro2019\.
...
.PARAMETER ConfigurationPath
    Path to Install-ProjectPro2024-<arch>.xml. Defaults to the file beside this script.

.PARAMETER SetupPath
    Path to setup.exe (Office Deployment Tool). Defaults to the file beside this script.

.EXAMPLE
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\Install-ProjectPro2024.ps1
    Auto-detects architecture and installs Project Professional 2024 in the OS language (MatchOS).

.EXAMPLE
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\Install-ProjectPro2024.ps1 -Architecture x86
    Forces the 32-bit config.

.NOTES
    - Intune Win32 app install command; runs as System.
    - Product installed: ProjectPro2024Volume (perpetual volume, KMS activation).
...
if (-not $ConfigurationPath) {
    $ConfigurationPath = Join-Path $scriptBase "Install-ProjectPro2024-$arch.xml"
...
if (-not (Test-Path -LiteralPath $ConfigurationPath)) {
    Write-Log -Message "Configuration file not found: $ConfigurationPath (verify Install-ProjectPro2024-$arch.xml is packaged beside this script)" -Level 'ERROR'
    exit 1
}

Write-Log -Message "Installing Project Professional 2024 ($arch, MatchOS) with: $ConfigurationPath" -Level 'INFO'
...
try {
    ...
    if ($exitCode -eq 0) {
        Write-Log -Message 'Project Professional 2024 installed successfully.' -Level 'SUCCESS'
    } else {
