...
.DESCRIPTION
    PowerShell wrapper for `setup.exe /configure Install-ProjectPro2024-<arch>-<lang>.xml` designed to run
...
.PARAMETER ConfigurationPath
    Path to Install-ProjectPro2024-<arch>-<lang>.xml. Defaults to the file beside this script.
...
.EXAMPLE
    ...
    Auto-detects architecture and installs Project Professional 2024 in the OS UI language (Auto).

.EXAMPLE
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\Install-ProjectPro2024.ps1 -Architecture x86 -Language Arabic
...
if (-not $ConfigurationPath) {
    $ConfigurationPath = Join-Path $scriptBase "Install-ProjectPro2024-$arch-$Language.xml"
...
if (-not (Test-Path -LiteralPath $ConfigurationPath)) {
    Write-Log -Message "Configuration file not found: $ConfigurationPath (verify Install-ProjectPro2024-$arch-$Language.xml is packaged beside this script)" -Level 'ERROR'
    exit 1
}

Write-Log -Message "Installing Project Professional 2024 ($arch, $Language) with: $ConfigurationPath" -Level 'INFO'
