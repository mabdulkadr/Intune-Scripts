...
**[Office LTSC Standard 2024 — silent, volume-licensed deployment package for Intune Win32 apps.]**
...
**Office LTSC 2024 - Intune Win32 Deployment** deploys Office LTSC Standard 2024 (volume licensed, `Standard2024Volume`) to Intune-managed Windows devices using the official Microsoft Office Deployment Tool, matching the configuration generated on [config.office.com](https://config.office.com).

The bundle contains three layers:

- **Configuration**: four XML variants covering every combination of architecture (64-bit / 32-bit) and language (English `en-us`, Arabic `ar-sa`) — `Install-Standard2024-<arch>-<lang>.xml`. `Uninstall-Standard2024.xml` describes what to remove.
...
- **Detection**: `Detect-OfficeLTSC2024.ps1` is the Intune Win32 custom detection rule that marks the app *Installed* only when the Click-to-Run configuration truly reports `Standard2024Volume` with a version, and a core binary exists on disk.
...
├── Install-Standard2024-x64-English.xml    Silent install - Standard2024Volume / en-us / 64-bit
├── Install-Standard2024-x64-Arabic.xml     Silent install - Standard2024Volume / ar-sa / 64-bit
├── Install-Standard2024-x86-English.xml    Silent install - Standard2024Volume / en-us / 32-bit
├── Install-Standard2024-x86-Arabic.xml     Silent install - Standard2024Volume / ar-sa / 32-bit
├── Uninstall-Standard2024.xml              Product removal config
...
### Purpose
Reads the authoritative Click-to-Run registry and reports whether Office LTSC Standard 2024 is genuinely present. Read-only — never modifies the system.

### Logic
1. Reads `HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration` (`ErrorAction Stop` → exit `2` if missing/failed).
2. Confirms `ProductReleaseIds` contains `Standard2024Volume` (exit `1` if not).
...
**Files**
```powershell
Install-OfficeLTSC2024.ps1   →  & setup.exe /configure Install-Standard2024-<arch>-<lang>.xml
Uninstall-OfficeLTSC2024.ps1 →  & setup.exe /configure Uninstall-Standard2024.xml
```
...
### Install-Standard2024-x64-English.xml Highlights
| Setting | Value | Purpose |
| --- | --- | --- |
| Channel | `PerpetualVL2024` | Volume channel for Office LTSC 2024 |
| Product / PIDKEY | `Standard2024Volume` + placeholder KMS GVLK | Volume activation, `AUTOACTIVATE=1` |
...
**Alternative (no script):** Registry rule → `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Office\ClickToRun\Configuration` → value `ProductReleaseIds` → **contains** `Standard2024Volume`.
...
5. ODT executes `setup.exe /configure Install-Standard2024-<arch>-<lang>.xml` silently (`Display None`, EULA accepted).
...
| --- | --- |
| Install returns `0x80070002` | ODT cannot find the config/source — confirm the folder was packaged whole (`setup.exe` + the selected `Install-Standard2024-*.xml` beside the install script) |
...
| Wrong product installed (`O365ProPlusRetail` shows in registry) | Channel/Product IDs from an earlier config — only `PerpetualVL2024` / `Standard2024Volume` produce LTSC Standard 2024 |
...
No. This configuration is LTSC-2024-only (`Standard2024Volume`). For M365 Apps, generate a retail config, e.g. via [config.office.com](https://config.office.com).
...
* **Offline first-install:** the packaged folder contains only `setup.exe` — the first `/configure` streams the payload from the CDN. For a self-contained package, run `setup.exe /download Install-Standard2024-x64-English.xml` first, then repackage with `IntuneWinAppUtil.exe` or [IntuneWin-Utility](https://github.com/mabdulkadr/IntuneWin-Utility).
