...
.TAGS
    Intune,Win32App,Office,LTSC,Deployment,Uninstall
...
.EXAMPLE
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\Uninstall-OfficeLTSC2024.ps1
...
# --- Main --------------------------------------------------------------------
$null = Initialize-Log -SolutionName $SolutionName -Type 'Intune'
Write-Banner
...
Write-Log -Message "Removing Office LTSC 2024 with: $ConfigurationPath" -Level 'INFO'
...
try {
    ...
    if ($exitCode -eq 0) {
        Write-Log -Message 'Office LTSC 2024 removed successfully.' -Level 'SUCCESS'
    } else {
