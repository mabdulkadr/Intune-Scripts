...
.DESCRIPTION
    ...
    complete effective surface for members of the group. Supports both interactive sign-in (delegated) and app-only authentication (client secret or certificate) on the workstation. Workstation-only execution.
...
.NOTES
    ...
    - Logs: %ProgramData%\get-group-assignments\Logs\
#>
...
# MAIN SCRIPT LOGIC
# ============================================================================
# Flow: log init -> banner -> resolve group -> scan surfaces -> report -> export.
...
try {
    ...
    Write-Log -Message "Group assignment scan started" -Level 'INFO'

    # ----- Resolve the group -----
    if ($GroupLookupMode -eq "ByName") {
        Write-Output "Resolving group '$GroupName'..."
        $escapedName = $GroupName -replace "'", "''"
        $groups = Get-MgGraphAllPages -Uri "https://graph.microsoft.com/beta/groups?`$filter=displayName eq '$escapedName'&`$select=id,displayName"
    ...
    Write-Log -Message "Group assignment scan completed: $($results.Count) assignments found" -Level 'SUCCESS'
}
catch {
