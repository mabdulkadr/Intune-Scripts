<#
.TITLE
    Detection - Configured Service Presence

.SYNOPSIS
    Checks whether the configured Windows service exists or appears to be running.

.DESCRIPTION
    Template detector that looks for the service defined in $servicename. It
    preserves the original loose detection behavior: the script considers the
    service compliant when its internal counter is non-zero, which means the
    service merely existing is enough to satisfy the check even when it is not
    running. Only a service that cannot be found is reported non-compliant. This
    script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Services
...
.PERMISSIONS
    None (local SYSTEM context) - reads one service status via Get-Service.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Restart-SystemService\Restart-SystemService-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        # Preserved loose template behavior: any non-zero counter counts as compliant.
        $checkarray   = 0
        $serviceExist = Get-Service -Name $servicename -ErrorAction SilentlyContinue

        if ($serviceExist) {
            $checkarray++
            Write-Log -Message "Configured service exists: $servicename" -Level 'SUCCESS'

            if ($serviceExist.Status -eq 'Running') {
                $checkarray++
                Write-Log -Message "Configured service is running: $servicename" -Level 'SUCCESS'
            }
            else {
                Write-Log -Message "Configured service is not running. Current state: $($serviceExist.Status)" -Level 'WARNING'
            }
        }
        else {
            Write-Log -Message "Configured service was not found: $servicename" -Level 'WARNING'
            $reasons.Add("Configured service was not found: $servicename")
        }
    }
    catch {
        throw "Failed to evaluate service presence: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - the template considers the service available or running" -Level 'SUCCESS'
    }
    ...
    Finish-Script -ExitCode 1 -Message "Non-compliant - The template considers the service non-compliant." -Level 'WARNING'
}
catch {
