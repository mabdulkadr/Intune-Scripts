<#
.TITLE
    Detection - PnP Devices in Error State

.SYNOPSIS
    Detects present PnP devices that report an error state.

.DESCRIPTION
    Evaluates Plug and Play devices with Get-PnpDevice and collects every
    present device reporting Status ERROR. Optional include and exclude filters
    can be applied to both device class and device ID. Any matching problem
    device makes the device non-compliant so the paired remediation can remove
    and re-scan it with pnputil.exe. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,PnP,Devices
...
.PERMISSIONS
    None (local SYSTEM context) - queries PnP device status only.
...
.NOTES
    ...
    - Keep detection under 30 seconds: single Get-PnpDevice query only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Test-PnPDeviceStatus\Test-PnPDeviceStatus-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        # Legacy summary cap preserved: list up to five devices, aggregate the rest.
        $matchedDevices = @(Get-MatchingPnpDevices)

        foreach ($device in ($matchedDevices | Select-Object -First 5)) {
            $deviceName = if ($device.FriendlyName) { $device.FriendlyName } else { $device.InstanceId }
            $reasons.Add("PnP device in error state: $deviceName")
        }

        if ($matchedDevices.Count -gt 5) {
            $reasons.Add("$($matchedDevices.Count - 5) additional PnP device(s) in error state were found")
        }
    }
    catch {
        throw "Failed to query PnP device status: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - No matching PnP devices in error state were found" -Level 'SUCCESS'
    }
