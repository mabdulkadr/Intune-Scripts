<#
.TITLE
    Remediation - Immediate DNS Client Cache Flush

.SYNOPSIS
    Flushes the Windows DNS client cache immediately by calling Clear-DnsClientCache.

.DESCRIPTION
    Paired remediation for Clear-DnsClientCacheImmediate. Runs whenever Intune executes it
    because the paired detector intentionally always reports non-compliant. Performs: (1) pre-remediation
    validation, (2) cache flush with failure tracking, (3) post-remediation verification,
...
.TAGS
    Remediation,Action,Maintenance,DNS,Cache
...
.PERMISSIONS
    None (local SYSTEM context) - flushes the local Windows DNS client cache.
...
.EXAMPLE
    ...
    Flushes the DNS client cache and verifies the cache is queryable; exits 0 on success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Clear-DnsClientCacheImmediate\Clear-DnsClientCacheImmediate-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The DnsClient module must expose Clear-DnsClientCache before the flush can run.
        if (-not (Get-Command -Name 'Clear-DnsClientCache' -ErrorAction SilentlyContinue)) {
            throw "Required command not found: Clear-DnsClientCache"
        }
...
function Test-FixApplied {
    try {
        # After a successful flush the DNS client cache must be queryable again;
        # Get-DnsClientCache succeeding proves the client cache is in a healthy state.
        $null = Get-DnsClientCache -ErrorAction Stop
        return $true
    }
    catch {
        Write-RemediationLog "Verification could not query the DNS client cache: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    Write-RemediationLog "Flushing the DNS client cache." -Level 'Info'

    $targetCount++
    Invoke-FixTarget -TargetName 'DNS client cache' -Fix {
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "DNS client cache flushed successfully." -Level 'SUCCESS'
    }
    else {
