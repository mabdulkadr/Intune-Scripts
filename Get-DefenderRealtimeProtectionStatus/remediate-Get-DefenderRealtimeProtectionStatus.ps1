<#
.TITLE
    Remediation - Enable Defender Real-Time Protection

.SYNOPSIS
    Enables Microsoft Defender real-time protection via Set-MpPreference.

.DESCRIPTION
    Paired remediation for Get-DefenderRealtimeProtectionStatus. Runs only when
    detect-Get-DefenderRealtimeProtectionStatus.ps1 returns exit 1. Performs:
    (1) pre-remediation validation, (2) Set-MpPreference -DisableRealtimeMonitoring $false,
...
.TAGS
    Remediation,Action,Defender,RealTimeProtection
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-DefenderRealtimeProtectionStatus\Get-DefenderRealtimeProtectionStatus-Remediation.txt
#>
...
$SolutionName = 'Get-DefenderRealtimeProtectionStatus'
...
function Test-RemediationPrerequisites {
    try {
        ...
        $script:RemediationResult.PreCheckStatus += "Current RealTimeProtectionEnabled value: $($status.RealTimeProtectionEnabled)"
        Write-RemediationLog "Current RealTimeProtectionEnabled value: $($status.RealTimeProtectionEnabled)" -Level 'Info'
...
function Test-FixApplied {
    try {
        ...
        $status = Get-MpComputerStatus -ErrorAction Stop
        return ($status.RealTimeProtectionEnabled -eq $true)
    }
    catch {
        Write-RemediationLog "Verification could not read RealTimeProtectionEnabled: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "Enable real-time protection" -Fix {
