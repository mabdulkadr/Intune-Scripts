<#
.TITLE
    Detection - Defender Real-Time Protection Status

.SYNOPSIS
    Verifies that Microsoft Defender real-time protection is enabled.

.DESCRIPTION
    Evaluates whether Microsoft Defender reports RealTimeProtectionEnabled = True
    via Get-MpComputerStatus. Real-time protection is the core on-access scanning
    layer of Microsoft Defender, so enterprise baselines require it enabled.
...
.TAGS
    Remediation,Detection,Defender,RealTimeProtection
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-DefenderRealtimeProtectionStatus\Get-DefenderRealtimeProtectionStatus-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        Write-Log -Message "Current RealTimeProtectionEnabled value: $($status.RealTimeProtectionEnabled)" -Level 'DEBUG'

        if ($status.RealTimeProtectionEnabled -ne $ExpectedValue) {
            $reasons.Add("Microsoft Defender real-time protection is not enabled - expected RealTimeProtectionEnabled=True, found $($status.RealTimeProtectionEnabled)")
        }
    }
    catch {
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Defender real-time protection is enabled" -Level 'SUCCESS'
    }
