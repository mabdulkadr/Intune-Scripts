<#
.TITLE
    Custom Compliance - Application Version
...
.DESCRIPTION
    ...
    otherwise the installed display version ('0.0.0.0' when absent). Defaults to
    version mode so the paired JSON rule file can enforce a minimum version.

    The related Get-AppVersionCompliance.json rule file can define:
...
.EXAMPLE
    .\Get-AppVersionCompliance.ps1
    Emits {"Google Chrome Installed":true} in install-check mode.
...
.NOTES
    ...
    - Logs: C:\ProgramData\Get-AppVersionCompliance\Logs
#>
...
$SolutionName = 'Get-AppVersionCompliance'
