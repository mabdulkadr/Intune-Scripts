<#
.TITLE
    Detection - SMBv1 Protocol Disabled

.SYNOPSIS
    Verifies that the legacy SMBv1 server protocol is disabled on the device.

.DESCRIPTION
    Evaluates whether SMBv1 is disabled by reading the EnableSMB1Protocol
    property from the SMB server configuration (Get-SmbServerConfiguration).
    SMBv1 is an outdated, insecure protocol targeted by wormable exploits such
    as EternalBlue, so enterprise baselines require it disabled. This script
...
.TAGS
    Remediation,Detection,SMBv1,Security
...
.PERMISSIONS
    None (local SYSTEM context) - reads the SMB server configuration only.
...
.NOTES
    ...
    - Keep detection under 30 seconds: single SMB server configuration query only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Disable-SMBv1Protocol\Disable-SMBv1Protocol-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        throw "Failed to read SMB server configuration: $($_.Exception.Message)"
    }

    $smbv1Enabled = $smbConfig.EnableSMB1Protocol
    Write-Log -Message "Current EnableSMB1Protocol value: $smbv1Enabled" -Level 'DEBUG'

    if ($smbv1Enabled) {
        $reasons.Add("SMBv1 server protocol is enabled - expected EnableSMB1Protocol=False, found $smbv1Enabled")
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - SMBv1 server protocol is already disabled" -Level 'SUCCESS'
    }
