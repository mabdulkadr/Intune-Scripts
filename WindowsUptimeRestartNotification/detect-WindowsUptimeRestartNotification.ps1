<#
.TITLE
    Detection - Windows Restart Notification Due

.SYNOPSIS
    Flags devices with a pending servicing reboot or uptime beyond the restart threshold.

.DESCRIPTION
    Evaluates two signals and reports non-compliance when either is true: (1) a Windows
    Update / CBS / PendingFileRenameOperations reboot is already pending, or (2) device
    uptime has reached the configured threshold. Non-compliance makes Intune run the
    paired user-facing restart notification. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Uptime,Restart
...
.PERMISSIONS
    None (local SYSTEM context) - reads registry keys and Win32_OperatingSystem only.
...
.NOTES
    ...
    - Keep detection under 30 seconds: registry probes plus one CIM query.
    - Unreadable uptime alone is treated as compliant to avoid false positives.
    - Logs: <SystemDrive>\IntuneLogs\WindowsUptimeRestartNotification\WindowsUptimeRestartNotification-Detection.txt
#>
...
# Keep this value aligned with the remediation scripts.
$MaxUptimeDays = 14
...
function Test-ComplianceState {
    ...
    try {
        ...
        if ($pendingRename -and $pendingRename.PendingFileRenameOperations) {
            $pendingSources += 'Pending File Rename'
        }
    }
    ...
    foreach ($source in $pendingSources) {
        $reasons.Add("A restart is already pending due to Windows servicing ($source)")
    }

    # --- CHECK 2: uptime threshold ------------------------------------------
    $uptimeDays = $null
    try {
        $operatingSystem = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction Stop
        $uptimeDays = [math]::Floor(((Get-Date) - $operatingSystem.LastBootUpTime).TotalDays)
        Write-Log -Message "Current uptime: $uptimeDays day(s)" -Level 'DEBUG'
    }
    catch {
        throw "Failed to read device uptime: $($_.Exception.Message)"
    }

    if (($null -ne $uptimeDays) -and ($uptimeDays -ge $MaxUptimeDays)) {
        $reasons.Add("Device uptime is $uptimeDays day(s) - meets or exceeds the $MaxUptimeDays day threshold")
    }
...
try {
    ...
    Write-Log -Message "Maximum uptime threshold: $MaxUptimeDays day(s)" -Level 'DEBUG'

    $reasons = Test-ComplianceState

    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - no pending reboot and uptime within threshold" -Level 'SUCCESS'
    }
