<#
.TITLE
    Remediation - Enable .NET Framework 3.5

.SYNOPSIS
    Enables the .NET Framework 3.5 (NetFx3) Windows optional feature.

.DESCRIPTION
    Paired remediation for Install-DotNetFramework35. Runs only when
    detect-Install-DotNetFramework35.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) Enable-WindowsOptionalFeature for NetFx3 with -All -NoRestart,
    (3) post-remediation verification, (4) structured JSON result output for Intune
    diagnostics. Intended for devices that still run legacy applications requiring .NET 3.5.
...
.TAGS
    Remediation,Action,NetFx3,WindowsFeatures
...
.PERMISSIONS
    None (local SYSTEM context) - enables a Windows optional feature via DISM.
...
.EXAMPLE
    ...
    Enables NetFx3 and verifies the resulting state; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Install-DotNetFramework35\Install-DotNetFramework35-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The DISM-backed cmdlets must exist before the fix can run.
        foreach ($commandName in @('Enable-WindowsOptionalFeature', 'Get-WindowsOptionalFeature')) {
        ...
        $script:RemediationResult.PreCheckStatus += "Current NetFx3 state: $($feature.State)"
        Write-RemediationLog "Current NetFx3 state: $($feature.State)" -Level 'Info'
...
function Test-FixApplied {
    try {
        # Re-check the same state the detector read. Return $true only when the
        # observed state now matches the compliant definition.
        $feature = Get-WindowsOptionalFeature -Online -FeatureName $FeatureName -ErrorAction Stop
        return ($feature.State -eq $ExpectedState)
    }
    catch {
        Write-RemediationLog "Verification could not read NetFx3 feature state: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "Enable feature $FeatureName" -Fix {
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message ".NET Framework 3.5 was enabled successfully." -Level 'SUCCESS'
    }
    else {
