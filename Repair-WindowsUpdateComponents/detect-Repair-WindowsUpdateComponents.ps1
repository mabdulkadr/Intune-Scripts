<#
.TITLE
    Detection - Windows Update Recency

.SYNOPSIS
    Detects whether the last installed Windows update is recent enough.

.DESCRIPTION
    Evaluates whether the most recent installed Windows update (by InstalledOn
    date from Get-HotFix) is younger than the configured threshold of 40 days.
    A device with no installed updates carrying a valid InstalledOn date is
    reported non-compliant so the paired remediation can attempt recovery. This
...
.TAGS
    Remediation,Detection,WindowsUpdate
...
.PERMISSIONS
    None (local SYSTEM context) - queries installed hotfixes via Get-HotFix.
...
.NOTES
    ...
    - Threshold: the last installed update must be newer than UpdateThresholdDays = 40 days.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Repair-WindowsUpdateComponents\Repair-WindowsUpdateComponents-Detection.txt
#>
...
# Returns the newest InstalledOn date across installed hotfixes, or $null.
...
function Test-ComplianceState {
    ...
    try {
        Write-Log -Message "Update age threshold (days): $UpdateThresholdDays" -Level 'DEBUG'

        $lastUpdate = Get-LatestInstalledUpdateDate
        if ($null -eq $lastUpdate) {
            # Preserved legacy outcome: no dated updates is non-compliance, not a crash.
            $reasons.Add('No installed Windows updates with a valid InstalledOn date were found')
            return @($reasons)
        }

        Write-Log -Message "Last installed update date: $($lastUpdate.ToString('yyyy-MM-dd'))" -Level 'DEBUG'

        $daysSinceUpdate = (New-TimeSpan -Start $lastUpdate -End (Get-Date)).Days
        Write-Log -Message "Days since last update: $daysSinceUpdate" -Level 'DEBUG'

        if ($daysSinceUpdate -ge $UpdateThresholdDays) {
            $reasons.Add("The last update was installed $daysSinceUpdate days ago, which exceeds the threshold of $UpdateThresholdDays days")
        }
    }
    catch {
        throw "Failed to evaluate Windows Update recency: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Windows Update recency is compliant" -Level 'SUCCESS'
    }
