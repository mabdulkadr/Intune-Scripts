...
* `rebootNow` remote action by device names, IDs, or Entra group membership
* Throttled retries with 60-second back-off on 429 and per-device delays
* Portal-safe parameters and explicit confirmation handling
* Supports `-WhatIf` and `-RestartDelaySeconds` for controlled rollouts
* Per-device outcome logging with summary counts
...
### Logging
* `C:\ProgramData\restart-devices\Logs`
