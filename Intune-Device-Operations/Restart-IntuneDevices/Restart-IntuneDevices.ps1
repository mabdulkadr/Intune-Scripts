...
.CHANGELOG
    ...
    1.2 - Added -WhatIf dry run support; exit code 1 when any restart fails; 429 retry with 60s wait on restart calls; group matching now falls back to userPrincipalName/mail so user-membership groups work; group lookup failures abort with a distinct error; added field projections (`$select`) to managed device queries
...
try {
    ...
    Write-Log -Message "Remote restart operation started (target mode: $TargetMode)" -Level 'INFO'
    ...
    if ($DryRun) {
        ...
        Write-Log -Message "Dry run completed - no restart actions were sent" -Level 'INFO'
        exit 0
    }
    ...
    if (-not $Force -and -not $WhatIfPreference) {
        ...
        if ($confirmation -ne 'CONFIRM') {
            ...
            Write-Log -Message "Restart operation cancelled by user" -Level 'WARNING'
            exit 0
        }
    }
    ...
    if ($failedRestarts -gt 0) {
        ...
        Write-Log -Message "Restart operation finished with $failedRestarts failure(s) out of $($targetDevices.Count) device(s)" -Level 'WARNING'
        exit 1
    }
    ...
    Write-Log -Message "Restart operation completed successfully - $successfulRestarts device(s) scheduled for restart" -Level 'SUCCESS'
