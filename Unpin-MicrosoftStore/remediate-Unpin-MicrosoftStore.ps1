<#
.TITLE
    Remediation - Unpin Microsoft Store From Taskbar

.SYNOPSIS
    Invokes the shell verb that removes Microsoft Store from the taskbar.

.DESCRIPTION
    Paired remediation for Unpin-MicrosoftStore. Runs only when
    detect-Unpin-MicrosoftStore.ps1 returns exit 1. Opens the AppsFolder shell
    namespace through Shell.Application, resolves the Microsoft Store app item
    by wildcard name, finds the shell verb that unpins it from the taskbar,
    and invokes that verb. Performs: (1) pre-remediation validation, (2) verb
    invocation with failure tracking, (3) post-remediation verification, (4)
...
.TAGS
    Remediation,Action,Taskbar,MicrosoftStore
...
.PERMISSIONS
    None (local SYSTEM context) - invokes a shell verb in the AppsFolder namespace.
...
.EXAMPLE
    ...
    Invokes the unpin taskbar verb and verifies; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Unpin-MicrosoftStore\Unpin-MicrosoftStore-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The shell namespace must open and the Store item must resolve before
        # any verb can be invoked.
        $script:StoreItem = Get-StoreShellItem

        if (-not $script:StoreItem) {
            Write-RemediationLog "Microsoft Store shell item was not found. Nothing to remediate." -Level 'Info'
        }
        else {
            $unpinVerb = Get-UnpinVerb -ShellItem $script:StoreItem
            if (-not $unpinVerb) {
                Write-RemediationLog "Microsoft Store does not expose an unpin taskbar action. Nothing to remediate." -Level 'Info'
            }
            else {
                Write-RemediationLog "Microsoft Store resolved with an available unpin taskbar action" -Level 'Info'
            }
        }
...
function Test-FixApplied {
    try {
        # Re-run the same inspection the detector used. Return $true only when
        # the Store item is gone or no longer exposes an unpin taskbar verb.
        $item = Get-StoreShellItem
        if (-not $item) {
            return $true
        }

        return ($null -eq (Get-UnpinVerb -ShellItem $item))
    }
    catch {
        Write-RemediationLog "Verification could not inspect the taskbar pin state: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount        = $(if ($script:StoreItem) { 1 } else { 0 })

    Write-RemediationLog "Executing remediation actions..." -Level 'Info'

    if (-not $script:StoreItem) {
        # Absence of the Store item means there is nothing pinned to remove.
        Write-RemediationLog "No target item present - skipping verb invocation" -Level 'Info'
    }
    else {
        Invoke-FixTarget -TargetName $TargetAppPattern -Fix {
            $unpinVerb = Get-UnpinVerb -ShellItem $script:StoreItem
            if (-not $unpinVerb) {
                # The verb can disappear between pre-check and fix - that is the desired end state.
                throw 'No unpin taskbar action is exposed for Microsoft Store.'
            }

            Write-RemediationLog "Invoking Microsoft Store unpin taskbar action" -Level 'Info'
            $unpinVerb.DoIt()
        }
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Microsoft Store was unpinned from the taskbar successfully" -Level 'SUCCESS'
    }
    else {
