...
.NOTES
    ...
    - User principal names are resolved from the userId on the event whenever possible
...
try {
    ...
    $events = Get-MgGraphAllPages -Uri "https://graph.microsoft.com/beta/deviceManagement/troubleshootingEvents?`$filter=eventDateTime ge $cutoffDate"
    ...
    if ($IncludeAutopilotEvents) {
        ...
        $autopilotEvents = Get-MgGraphAllPages -Uri "https://graph.microsoft.com/beta/deviceManagement/autopilotEvents"
