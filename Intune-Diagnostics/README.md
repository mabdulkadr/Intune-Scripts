...
Adapted from [ugurkocde/IntuneAutomation](https://github.com/ugurkocde/IntuneAutomation). See [THIRD-PARTY-NOTICES.md](../../THIRD-PARTY-NOTICES.md) for full upstream license, preserved `.AUTHOR`/`.CHANGELOG` fields, and excluded Azure-only assets. **Clarification — Key Vault destination note:** The vault-destination pattern (`Azure Key Vault` as a `https://vault.azure.net` **RESOURCE**, not an Azure Automation runbook) applies only to `Intune-Security-Tools/Backup-BitLockerKeysToKeyVault.ps1`; no script in **this** Diagnostics folder touches Azure Key Vault or any vault-destination resource.
