...
##Download it
Invoke-WebRequest -Uri $PathToHTMLTemplate -OutFile "$PSScriptRoot\email-template.html"
$EmailContent = Get-Content "$PSScriptRoot\email-template.html" | Out-String
...
$tempemail | out-file "$PSScriptRoot\finalemail.html"

$EmailContent2 = Get-Content "$PSScriptRoot\finalemail.html" | Out-String
...
$SmtpPort     = 587
$SmtpUsername = 'noreplay@osrah.sa'
$SmtpPassword = 'Nn123osrah'  # For production, please use a secure method to store your password
...
# Replace the following variables with your actual ones
$EmailSubject  = "Daily Intune Report for $domain at $readabledate"
$EmailFrom     = "noreplay@osrah.sa"
$EmailTo       = "m.omar@osrah.sa"
$EmailBody     = $EmailContent2  # This is HTML content

###############################################################################
# Create a secure credential from the plain-text password
###############################################################################
