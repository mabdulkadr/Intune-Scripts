...
This script connects to Microsoft Graph, retrieves managed devices and their compliance status, and generates a detailed report in both CSV and HTML formats. The report includes device details, compliance status, and summary statistics. Supports interactive sign-in and unattended app-only via -TenantId/-ClientId.
...
**Get Device Compliance Report** is a PowerShell reporting script that This script connects to Microsoft Graph, retrieves managed devices and their compliance status, and generates a detailed report in both CSV and HTML formats. The report includes device details, compliance status, and summary statistics. Supports interactive sign-in and unattended app-only via -TenantId/-ClientId.
...
### Example 2
```powershell
.\Get-DeviceComplianceReport.ps1 -OutputPath "C:\Reports"
```
Generates reports and saves them to the specified directory

### Example 3
```powershell
.\Get-DeviceComplianceReport.ps1 -ForceModuleInstall "true"
```
...
|-----------|------|----------|---------|-------------|
| `OutputPath` | string | No | - | Output directory for the reports |
...
### Logging
* `C:\ProgramData\get-device-compliance-report\Logs\`
