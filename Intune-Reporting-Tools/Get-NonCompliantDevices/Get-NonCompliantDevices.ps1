...
.NOTES
    ...
    - Logs: %ProgramData%\get-noncompliant-devices-with-reasons\Logs\<timestamp>.log
#>
...
try {
    ...
    Write-Log -Message "Starting non-compliant device reason report" -Level 'INFO'
    ...
    Write-Log -Message "Found $($devices.Count) device(s) matching the requested compliance state(s)" -Level 'INFO'
    ...
    foreach ($device in $devices) {
        ...
        try {
            ...
            $policyStates = Get-MgGraphAllPages -Uri $policyStatesUri
            ...
            foreach ($policy in $failingPolicies) {
                try {
                    ...
                    $settingStates = Get-MgGraphAllPages -Uri $settingStatesUri
                    ...
                    # Setting-level detail is best-effort per policy - log and continue.
                    Write-Log -Message "Error retrieving setting states for device '$($device.deviceName)' policy '$($policy.displayName)': $($_.Exception.Message)" -Level 'WARNING'
    ...
    Write-Log -Message "Collected $($report.Count) reason row(s) across $distinctDevices device(s)" -Level 'INFO'
    ...
    Write-Log -Message "Non-compliant device reason report completed successfully" -Level 'SUCCESS'
}
catch {
