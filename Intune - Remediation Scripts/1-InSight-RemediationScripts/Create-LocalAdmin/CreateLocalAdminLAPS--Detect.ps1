<#
.SYNOPSIS
    Detect whether the configured LAPS local admin account already exists.
...
.DESCRIPTION
    ...
    so the LAPS remediation script does not need to create it.

.RUN AS
    System

.EXAMPLE
    .\CreateLocalAdminLAPS--Detect.ps1
...
$ScriptName     = 'CreateLocalAdminLAPS--Detect.ps1'
$ScriptBaseName = 'CreateLocalAdminLAPS--Detect'
...
if (Get-LocalUser | Where-Object Name -eq $LocalAdminName) {
    ...
    Write-Log -Level 'OK' -Message 'The configured LAPS local admin account already exists.'
    Write-Log -Level 'INFO' -Message '=== Detection END (Exit 0) ==='
    Exit 0
}
else {
    Write-Host 'User does not exist'
    Write-Log -Level 'WARN' -Message 'The configured LAPS local admin account was not found.'
