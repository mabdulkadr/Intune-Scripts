<#
.SYNOPSIS
    Detect whether Microsoft Defender Network Protection is enabled.
...
.DESCRIPTION
    ...
    returns `0` only when Network Protection is enabled (`1`).

.RUN AS
    System

.EXAMPLE
    .\GetNetworkProtection--Detect.ps1
...
$ScriptName     = 'GetNetworkProtection--Detect.ps1'
$ScriptBaseName = 'GetNetworkProtection--Detect'
...
$SolutionName = 'GetNetworkProtection'
...
Write-Log -Message ("EnableNetworkProtection: {0}" -f $mpPreference.EnableNetworkProtection)

if ($mpPreference.EnableNetworkProtection -eq 1) {
    Write-Log -Message 'Microsoft Defender Network Protection is enabled.' -Level 'OK'
    Write-Log -Message '=== Detection END (Exit 0) ==='
    Write-Output "$version COMPLIANT"
    exit 0
}
else {
    Write-Log -Message 'Microsoft Defender Network Protection is not enabled.' -Level 'WARN'
