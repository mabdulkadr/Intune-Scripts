<#
.SYNOPSIS
    Detect whether Microsoft Defender behavior monitoring is enabled.
...
.DESCRIPTION
    ...
    Behavior monitoring is part of Microsoft Defender real-time protection and
    helps detect suspicious behavior from running processes.

    The original script reports compliance using the version marker `C1` and
    returns `0` only when `BehaviorMonitorEnabled` is `True`.

.RUN AS
    System

.EXAMPLE
    .\GetRealTimeBehaviour--Detect.ps1
...
$ScriptName     = 'GetRealTimeBehaviour--Detect.ps1'
$ScriptBaseName = 'GetRealTimeBehaviour--Detect'
...
$SolutionName = 'GetRealTimeBehaviour'
...
Write-Log -Message ("BehaviorMonitorEnabled: {0}" -f $mpStatus.BehaviorMonitorEnabled)

if ($mpStatus.BehaviorMonitorEnabled -eq 'True') {
    Write-Log -Message 'Microsoft Defender behavior monitoring is enabled.' -Level 'OK'
    Write-Log -Message '=== Detection END (Exit 0) ==='
    Write-Output "$version COMPLIANT"
    exit 0
}
else {
    Write-Log -Message 'Microsoft Defender behavior monitoring is not enabled.' -Level 'WARN'
