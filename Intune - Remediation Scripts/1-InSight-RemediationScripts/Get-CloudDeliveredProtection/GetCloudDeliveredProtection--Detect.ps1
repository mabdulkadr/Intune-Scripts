<#
.SYNOPSIS
    Detect whether Microsoft Defender cloud-delivered protection is fully enabled.

.DESCRIPTION
    This detection script checks two Microsoft Defender preferences:

    - `MAPSReporting` must equal `2`
    - `SubmitSamplesConsent` must equal `3`

    The original script reports compliance using the version marker `C1` and
    returns `0` only when both values match the expected configuration.

.RUN AS
    System

.EXAMPLE
    .\GetCloudDeliveredProtection--Detect.ps1
...
$ScriptName     = 'GetCloudDeliveredProtection--Detect.ps1'
$ScriptBaseName = 'GetCloudDeliveredProtection--Detect'
...
$SolutionName = 'GetCloudDeliveredProtection'
...
Write-Log -Message ("MAPSReporting: {0}" -f $mpPreference.MAPSReporting)
Write-Log -Message ("SubmitSamplesConsent: {0}" -f $mpPreference.SubmitSamplesConsent)

if (($mpPreference.MAPSReporting -eq 2) -and ($mpPreference.SubmitSamplesConsent -eq 3)) {
    Write-Log -Message 'Cloud-delivered protection is fully enabled.' -Level 'OK'
    Write-Log -Message '=== Detection END (Exit 0) ==='
    Write-Output "$version COMPLIANT"
    exit 0
}
else {
    Write-Log -Message 'Cloud-delivered protection is not configured as expected.' -Level 'WARN'
