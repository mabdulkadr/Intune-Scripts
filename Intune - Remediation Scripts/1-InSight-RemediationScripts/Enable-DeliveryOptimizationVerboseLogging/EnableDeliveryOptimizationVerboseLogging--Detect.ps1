<#
.SYNOPSIS
    Always trigger the remediation that enables Delivery Optimization verbose logging.

.DESCRIPTION
    This detection script is intentionally configured to always return a
    non-compliant result so the paired remediation script runs every time.

    It is used when the goal is to force `Enable-DeliveryOptimizationVerboseLogs`
    rather than to check an existing setting.
...
.EXAMPLE
    .\EnableDeliveryOptimizationVerboseLogging--Detect.ps1
...
$ScriptName     = 'EnableDeliveryOptimizationVerboseLogging--Detect.ps1'
$ScriptBaseName = 'EnableDeliveryOptimizationVerboseLogging--Detect'
...
# Always trigger, exactly like the original script.
