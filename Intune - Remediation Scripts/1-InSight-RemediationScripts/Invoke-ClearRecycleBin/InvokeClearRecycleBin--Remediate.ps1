<#
.SYNOPSIS
    Clear the current user's Recycle Bin contents.

.DESCRIPTION
    This remediation script runs the built-in PowerShell command:

        Clear-RecycleBin -Force

    to clear the Recycle Bin for the current user without confirmation prompts.

.RUN AS
    User

.EXAMPLE
    .\InvokeClearRecycleBin--Remediate.ps1
...
$ScriptName     = 'InvokeClearRecycleBin--Remediate.ps1'
$ScriptBaseName = 'InvokeClearRecycleBin--Remediate'
...
Write-Log -Message 'Clearing the current user recycle bin.'
Clear-RecycleBin -Force
Write-Log -Message 'Recycle Bin clear command completed.' -Level 'OK'
