<#
.SYNOPSIS
    Detect whether the AlwaysInstallElevated policy is disabled in HKLM.

.DESCRIPTION
    This detection script checks the machine policy value:

        HKLM:\SOFTWARE\Policies\Microsoft\Windows\Installer\AlwaysInstallElevated

    The device is compliant only when the value exists and equals `0`.
...
$ScriptName     = 'GetAlwaysElevated--Detect.ps1'
$ScriptBaseName = 'GetAlwaysElevated--Detect'
...
Try {
    ...
    if ($Registry -eq $Value) {
        Write-Log -Message 'AlwaysInstallElevated is disabled.' -Level 'OK'
        Write-Log -Message '=== Detection END (Exit 0) ==='
        Write-Output 'Compliant'
        Exit 0
    }

    Write-Log -Message 'AlwaysInstallElevated is not set to the expected value.' -Level 'WARN'
