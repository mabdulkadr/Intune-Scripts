<#
.SYNOPSIS
     Detect whether LSA Protection (LSASS Protected Process) is enabled.

.DESCRIPTION
    This detection script checks the registry value:

        HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\RunAsPPL

    The device is compliant only when `RunAsPPL` exists and equals `1`.
...
$ScriptName     = 'GetLSAProtection--Detect.ps1'
$ScriptBaseName = 'GetLSAProtection--Detect'
...
Try {
    ...
    if ($Registry -eq $Value) {
        Write-Log -Message 'LSA protection is enabled.' -Level 'OK'
        Write-Log -Message '=== Detection END (Exit 0) ==='
        Write-Output 'Compliant'
        Exit 0
    }

    Write-Log -Message 'LSA protection is not set to the expected value.' -Level 'WARN'
