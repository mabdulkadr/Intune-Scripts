<#
.SYNOPSIS
    Displays an English restart prompt and performs optional restart scheduling actions.

.DESCRIPTION
    This script is designed for Intune Proactive Remediations.
    It shows a custom WPF dialog when either condition is met:
    ...
    - Save as UTF-8 with BOM if you later reintroduce non-English text in PowerShell 5.1

    Exit codes:
    - Exit 0: Completed successfully or no action required
    - Exit 1: Failed to load the prompt window
...
$Txt_HeaderSubTitle = 'Please review this notice and choose the appropriate action'
$Txt_Footer         = 'For assistance, please contact the IT support team.'
$Txt_DeployedBy     = 'Sent via Microsoft Intune'
...
# Paste your current LogoBase64 value here if you use the embedded logo
...
# Build English message text
$Txt_MessageTitle = 'Device Restart Required'
$Sections = @()

if ($PendingInfo.Reasons.Count -gt 0) {
    $ReasonText = ' (' + ($PendingInfo.Reasons -join ', ') + ')'
    $Sections += "• System updates or changes require a restart$ReasonText."
}

if (($null -ne $UptimeDays) -and ($UptimeDays -ge $MaxUptimeDays)) {
    $Sections += "• Device uptime has exceeded $MaxUptimeDays days without a restart."
}

if ($Sections.Count -eq 0) {
    $Sections += '• A restart is required to complete system requirements.'
}
...
$MessageLines += '• Or schedule a restart in 1 hour.'
$MessageLines += '• Or schedule a restart in 2 hours.'
...
if ($BtnRestart1H) {
    $BtnRestart1H.Add_Click({
        ...
        Write-Log -Message "Restart in 1 hour clicked. Scheduled=$Scheduled"
...
if ($BtnRestart2H) {
    $BtnRestart2H.Add_Click({
        ...
        Write-Log -Message "Restart in 2 hours clicked. Scheduled=$Scheduled"
