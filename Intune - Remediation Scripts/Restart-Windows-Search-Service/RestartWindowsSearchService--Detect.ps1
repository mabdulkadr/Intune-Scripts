<#
.SYNOPSIS
    Detects whether the Windows Search service is installed and running.

.DESCRIPTION
    This detection script checks for the Windows Search service and verifies that
...
.EXAMPLE
    .\RestartWindowsSearchService--Detect.ps1
...
$ScriptName     = 'RestartWindowsSearchService--Detect.ps1'
$ScriptBaseName = 'RestartWindowsSearchService--Detect'

# Store logs under the system drive instead of hard-coding C:.
$SystemDrive   = if ($env:SystemDrive) { $env:SystemDrive } else { 'C:' }
$LogFolderName = 'Restart-Windows-Search-Service'
$LogDirectory  = Join-Path $SystemDrive "Intune\$LogFolderName"
$LogFilePath   = Join-Path $LogDirectory "$ScriptBaseName.txt"

# Target the Windows Search service by its service name.
$ServiceName = 'WSearch'
