<#
.TITLE
    Remediation - Show Reboot Reminder Toast

.SYNOPSIS
    Shows a simple toast asking the user to reboot the machine.

.DESCRIPTION
    Paired remediation for Show-RebootToastNotification. Runs only when
    detect-Show-RebootToastNotification.ps1 returns exit 1. Performs:
    (1) pre-remediation validation, (2) WinRT toast dispatch using the built-in
    ToastText02 template with the original message content, (3) post-remediation
...
.TAGS
    Remediation,Action,Reboot,Notification
...
.PERMISSIONS
    None (local SYSTEM context) - dispatches a Windows toast notification via WinRT.
...
.EXAMPLE
    ...
    Dispatches the reboot reminder toast and verifies dispatch; exits 0 on success.
...
.NOTES
    - Requires the signed-in user context for the toast to be visible, so deploy
      with "Run this script using logged-on credentials: Yes" in Intune.
    - Verification honesty: success means the toast dispatch call succeeded;
      user acknowledgment cannot be verified programmatically.
    - Idempotent: safe to run repeatedly; it re-sends the reminder each time.
    - Logs: <SystemDrive>\IntuneLogs\Show-RebootToastNotification\Show-RebootToastNotification-Remediation.txt
#>
...
# ============================================================================
# NOTIFICATION MECHANISM (original WinRT ToastText02 mechanism preserved)
# ============================================================================

# Builds and shows one toast with the original template, tag, group, and expiry.
...
function Test-RemediationPrerequisites {
    try {
        # The WinRT toast API must load before we promise the user a reminder.
...
function Test-FixApplied {
    try {
        # Verification honesty: success means the WinRT toast dispatch call
        # completed without error. User acknowledgment cannot be verified
        # programmatically.
        if (-not $script:ToastDispatched) {
            Write-RemediationLog "Verification failed - the reboot reminder toast was not dispatched" -Level 'Error'
            return $false
        }
        return $true
    }
...
try {
    ...
    $targetCount        = 0
    $script:ToastDispatched = $false

    Write-RemediationLog "Executing remediation actions..." -Level 'Info'

    $targetCount++
    Invoke-FixTarget -TargetName "Reboot reminder toast" -Fix {
        Write-RemediationLog "Showing reboot reminder toast notification." -Level 'Info'
        Show-Notification -Title $ToastTitle -Message $ToastText
        $script:ToastDispatched = $true
    }
    ...
    if ($verificationPassed) {
        ...
        $script:RemediationResult.PostCheckStatus += "Toast notification dispatch succeeded"
        ...
        Finish-Script -ExitCode 0 -Message "Reboot reminder toast notification was shown" -Level 'SUCCESS'
    }
    else {
