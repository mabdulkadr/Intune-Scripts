<#
.TITLE
    Remediation - Enable Automatic Time Zone

.SYNOPSIS
    Writes the registry values required for automatic time zone updates to work.

.DESCRIPTION
    Paired remediation for Set-AutomaticTimeZone. Runs only when
    detect-Set-AutomaticTimeZone.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) registry fixes with failure tracking, (3) post-remediation
    verification, (4) structured JSON result output for Intune diagnostics.
    Allows location access in the Capability Access Manager key and sets the
    tzautoupdate service Start value to 3.
...
.TAGS
    Remediation,Action,Timezone,Registry
...
.PERMISSIONS
    None (local SYSTEM context) - writes two HKLM registry values.
...
.EXAMPLE
    ...
    Applies both registry values and verifies them; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Set-AutomaticTimeZone\Set-AutomaticTimeZone-Remediation.txt
#>
...
function Get-RegistryValue {
    ...
    try {
        ...
        # Missing key or value is an expected pre-state before the fix is applied.
        return $null
    }
}
...
function Test-RemediationPrerequisites {
    try {
        # New-ItemProperty requires the parent keys to exist before it can write.
        if (-not (Test-Path -LiteralPath $LocationConsentPath)) {
            throw "Required registry key not found: $LocationConsentPath"
        }
        if (-not (Test-Path -LiteralPath $TimeZoneServicePath)) {
            throw "Required registry key not found: $TimeZoneServicePath"
        }
...
function Test-FixApplied {
    try {
        # Re-check the same values the detector read. Return $true only when the
        ...
        if ($locationValue -ne $LocationConsentValue) {
            Write-RemediationLog "Location consent verification failed after remediation" -Level 'Error'
            return $false
        }

        if ([int]$timeZoneValue -ne $TimeZoneServiceValue) {
            Write-RemediationLog "tzautoupdate verification failed after remediation" -Level 'Error'
            return $false
        }
        ...
        Write-RemediationLog "Verification could not read the time zone registry values: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Write-RemediationLog "Writing location consent value for automatic time zone." -Level 'Info'
    Invoke-FixTarget -TargetName "$LocationConsentName=Allow" -Fix {
        New-ItemProperty -LiteralPath $LocationConsentPath -Name $LocationConsentName -Value $LocationConsentValue -PropertyType $LocationConsentType -Force -ErrorAction Stop | Out-Null
    }

    $targetCount++
    Write-RemediationLog "Writing tzautoupdate service startup value." -Level 'Info'
    Invoke-FixTarget -TargetName "$TimeZoneServiceName=$TimeZoneServiceValue" -Fix {
        New-ItemProperty -LiteralPath $TimeZoneServicePath -Name $TimeZoneServiceName -Value $TimeZoneServiceValue -PropertyType $TimeZoneServiceType -Force -ErrorAction Stop | Out-Null
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Automatic time zone settings were applied successfully" -Level 'SUCCESS'
    }
    else {
