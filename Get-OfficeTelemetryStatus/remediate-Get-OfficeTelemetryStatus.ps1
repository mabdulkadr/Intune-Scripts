<#
.TITLE
    Remediation - Disable Office Client Telemetry

.SYNOPSIS
    Writes DisableTelemetry = 1 under the Office policy key for the current user.

.DESCRIPTION
    Paired remediation for Get-OfficeTelemetryStatus. Runs only when
    detect-Get-OfficeTelemetryStatus.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) creation of the clienttelemetry policy key and the DisableTelemetry
    DWORD value, (3) post-remediation verification, (4) structured JSON result output
    for Intune diagnostics.
...
.TAGS
    Remediation,Action,Office,Privacy
...
.PERMISSIONS
    None (local SYSTEM context) - writes one HKCU registry value.
...
.EXAMPLE
    ...
    Applies the policy value and verifies it; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-OfficeTelemetryStatus\Get-OfficeTelemetryStatus-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The HKU Policies hive must be reachable before any key is created.
        if (-not (Test-Path -LiteralPath 'HKCU:\Software\Policies\Microsoft')) {
            throw "Required parent registry path not found: HKCU:\Software\Policies\Microsoft"
        }
...
function Test-FixApplied {
    try {
        ...
        $currentValue = Get-ItemPropertyValue -LiteralPath $RegistryPath -Name $ValueName -ErrorAction Stop
        return ($currentValue -eq $ExpectedValue)
    }
    catch [System.Management.Automation.ItemNotFoundException] {
        Write-RemediationLog "Verification could not find ${RegistryPath}\${ValueName}: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
    catch {
        Write-RemediationLog "Verification could not read ${ValueName}: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $beforeValue = $null
    try {
        $beforeValue = Get-ItemPropertyValue -LiteralPath $RegistryPath -Name $ValueName -ErrorAction Stop
    }
    catch [System.Management.Automation.ItemNotFoundException] {
        # Value absent is an expected pre-state - it gets created by the fix below.
        Write-RemediationLog "$ValueName not currently set - it will be created" -Level 'Info'
    }
    if ($null -ne $beforeValue) {
        Write-RemediationLog "Current $ValueName value: $beforeValue" -Level 'Info'
    }

    $targetCount++
    Invoke-FixTarget -TargetName "$ValueName=$ExpectedValue" -Fix {
        if (-not (Test-Path -LiteralPath $RegistryPath)) {
            New-Item -Path $ParentPath -Name $KeyName -Force -ErrorAction Stop | Out-Null
            Write-RemediationLog "Created Office telemetry policy key." -Level 'Info'
        }

        New-ItemProperty -Path $RegistryPath -Name $ValueName -Value $ExpectedValue -PropertyType $ValueType -Force -ErrorAction Stop | Out-Null
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Office client telemetry policy was applied successfully for the current user." -Level 'SUCCESS'
    }
    else {
