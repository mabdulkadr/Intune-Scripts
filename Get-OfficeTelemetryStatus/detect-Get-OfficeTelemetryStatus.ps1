<#
.TITLE
    Detection - Office Client Telemetry Policy Status

.SYNOPSIS
    Verifies that Office client telemetry is disabled for the current user.

.DESCRIPTION
    Evaluates whether the DisableTelemetry policy value under
    HKCU:\Software\Policies\Microsoft\office\common\clienttelemetry equals 1.
    Disabling Office client telemetry reduces data sent to Microsoft from
    managed endpoints. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Office,Privacy
...
.PERMISSIONS
    None (local SYSTEM context) - reads one HKCU registry value.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-OfficeTelemetryStatus\Get-OfficeTelemetryStatus-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        Write-Log -Message "Current $ValueName value: $currentValue" -Level 'DEBUG'

        if ($currentValue -ne $ExpectedValue) {
            $reasons.Add("Office client telemetry is not disabled - expected $ValueName=$ExpectedValue, found $currentValue")
        }
    }
    catch [System.Management.Automation.ItemNotFoundException] {
        $reasons.Add("Registry value missing: $RegistryPath\$ValueName does not exist")
    }
    catch {
        throw "Failed to read Office telemetry policy: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Office client telemetry is disabled for the current user" -Level 'SUCCESS'
    }
