<#
.TITLE
    Remediation - Enable WinRM Service

.SYNOPSIS
    Enables the WinRM service, PowerShell Remoting, and validates local WSMan availability.

.DESCRIPTION
    Paired remediation for Enable-WinRMService. Runs only when
    detect-Enable-WinRMService.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) service configuration fixes with failure tracking,
    (3) post-remediation verification via a local WSMan test, (4) structured
...
.TAGS
    Remediation,Action,WinRM,Remoting
...
.PERMISSIONS
    None (local SYSTEM context) - configures and starts the WinRM service.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Enable-WinRMService\Enable-WinRMService-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The WinRM service entry must exist before its startup type or state
        # can be changed. A missing service cannot be corrected by this fix.
        $service = Get-Service -Name $ServiceName -ErrorAction Stop
        Write-RemediationLog "Service '$ServiceName' found with status '$($service.Status)'" -Level 'Info'
...
function Test-FixApplied {
    try {
        # Re-run the same probe the detector used. Return $true only when the
        # observed state now matches the compliant definition.
        Test-WSMan -ComputerName localhost -ErrorAction Stop | Out-Null
        return $true
    }
    catch {
        Write-RemediationLog "Verification could not complete the local WSMan test: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "$ServiceName startup type = Automatic" -Fix {
        $serviceInstance = Get-CimInstance -ClassName Win32_Service -Filter "Name='$ServiceName'" -ErrorAction Stop

        if ($serviceInstance.StartMode -ne 'Auto') {
            Write-RemediationLog "WinRM startup type is '$($serviceInstance.StartMode)'. Changing it to Automatic." -Level 'Info'
            Set-Service -Name $ServiceName -StartupType Automatic -ErrorAction Stop
            Write-RemediationLog "WinRM startup type set to Automatic" -Level 'Info'
        }
        else {
            Write-RemediationLog "WinRM startup type is already Automatic" -Level 'Info'
        }
    }

    $targetCount++
    Invoke-FixTarget -TargetName "$ServiceName service running" -Fix {
        $service = Get-Service -Name $ServiceName -ErrorAction Stop

        if ($service.Status -ne 'Running') {
            Write-RemediationLog "WinRM service is not running. Starting the service." -Level 'Info'
            Start-Service -Name $ServiceName -ErrorAction Stop
            $service.WaitForStatus('Running', (New-TimeSpan -Seconds 15))
            Write-RemediationLog "WinRM service started successfully" -Level 'Info'
        }
        else {
            Write-RemediationLog "WinRM service is already running" -Level 'Info'
        }
    }

    $targetCount++
    Invoke-FixTarget -TargetName "PowerShell Remoting enabled" -Fix {
        Enable-PSRemoting -Force -SkipNetworkProfileCheck -ErrorAction Stop | Out-Null
        Write-RemediationLog "PowerShell Remoting enabled with SkipNetworkProfileCheck" -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "WinRM remediation completed successfully" -Level 'SUCCESS'
    }
    else {
