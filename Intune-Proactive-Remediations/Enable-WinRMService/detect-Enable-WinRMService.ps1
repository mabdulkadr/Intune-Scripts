<#
.TITLE
    Detection - WinRM Service Enabled

.SYNOPSIS
    Verifies that WinRM (PowerShell Remoting) is enabled and responding locally.

.DESCRIPTION
    Evaluates whether WinRM and PowerShell Remoting are available by running
    Test-WSMan against the local device. A successful WSMan response means the
    device is compliant; a failure means WinRM appears to be disabled and the
    paired remediation should run. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,WinRM,Remoting
...
.PERMISSIONS
    None (local SYSTEM context) - runs one local Test-WSMan query.
...
.NOTES
    ...
    - Keep detection under 30 seconds: single local WSMan test only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Enable-WinRMService\Enable-WinRMService-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        Write-Log -Message "WSMan responded with product version: $($testResult.ProductVersion)" -Level 'DEBUG'

        if (-not $testResult) {
            $reasons.Add("Test-WSMan did not return a valid result")
        }
    }
    catch {
        # WSMan not responding is the expected non-compliant state while WinRM
        # is disabled - it is a compliance reason, not a script error.
        $reasons.Add("WinRM is disabled or unavailable - Test-WSMan failed: $($_.Exception.Message)")
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - WinRM is enabled and responding" -Level 'SUCCESS'
    }
