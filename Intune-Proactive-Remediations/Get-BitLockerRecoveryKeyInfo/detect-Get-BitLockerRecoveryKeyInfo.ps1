<#
.TITLE
    Detection - BitLocker Recovery Key Available

.SYNOPSIS
    Checks whether the OS volume exposes a BitLocker recovery password.

.DESCRIPTION
    Evaluates whether the C: volume exposes a BitLocker recovery password by
    querying Get-BitLockerVolume and inspecting the KeyProtector collection for
    a populated RecoveryPassword value. This package is status/informational in
    nature: compliance simply documents that a recoverable key exists, and a
    missing key routes the device to the paired reporting remediation. This
...
.TAGS
    Remediation,Detection,BitLocker,Security
...
.NOTES
    ...
    - Keep detection under 30 seconds: single BitLocker volume query only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Get-BitLockerRecoveryKeyInfo\Get-BitLockerRecoveryKeyInfo-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        throw "Failed to query BitLocker volume ${MountPoint}: $($_.Exception.Message)"
    }

    $recoveryPasswords = @($volume.KeyProtector | Where-Object { $_.RecoveryPassword } | Select-Object -ExpandProperty RecoveryPassword)
    Write-Log -Message "Recovery password key protectors found: $($recoveryPasswords.Count)" -Level 'DEBUG'

    if ($recoveryPasswords.Count -eq 0) {
        $reasons.Add("No BitLocker recovery password was found on the OS volume $MountPoint")
    }
...
try {
    ...
    Write-Log -Message "Querying BitLocker recovery password on mount point: $MountPoint" -Level 'INFO'

    $reasons = Test-ComplianceState

    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - BitLocker recovery password is available on the OS volume" -Level 'SUCCESS'
    }
