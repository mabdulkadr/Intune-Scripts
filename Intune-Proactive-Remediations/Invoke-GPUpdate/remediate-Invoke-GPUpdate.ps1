<#
.TITLE
    Remediation - Force Group Policy Update

.SYNOPSIS
    Forces a Group Policy refresh by running gpupdate /force.

.DESCRIPTION
    Paired remediation for Invoke-GPUpdate. Runs only when
    detect-Invoke-GPUpdate.ps1 returns exit 1. Performs: (1) pre-remediation
    validation that the gpupdate command resolves, (2) command invocation with
    failure tracking, (3) post-remediation verification of the command result,
...
.TAGS
    Remediation,Action,GroupPolicy,AlwaysTrigger
...
.PERMISSIONS
    None (local SYSTEM context) - runs gpupdate /force.
...
.EXAMPLE
    ...
    Runs gpupdate /force and verifies the command result; exits 0 on verified success.
...
.NOTES
    ...
    - Verification semantics: gpupdate exiting 0 confirms the refresh was initiated;
      Group Policy application itself is asynchronous and is not verified here.
    - Logs: <SystemDrive>\IntuneLogs\Invoke-GPUpdate\Invoke-GPUpdate-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The gpupdate executable must resolve before it is invoked.
...
function Test-FixApplied {
    # gpupdate exiting 0 confirms the refresh was initiated; Group Policy
    # application itself is asynchronous and cannot be verified synchronously.
    return ($script:GpupdateExitCode -eq 0)
}
...
try {
    ...
    $targetCount              = 0
    $script:GpupdateExitCode  = $null

    Write-RemediationLog "Executing remediation actions..." -Level 'Info'

    $targetCount++
    Invoke-FixTarget -TargetName "$CommandName $Arguments" -Fix {
        & $CommandName $Arguments | Out-Null
        $script:GpupdateExitCode = $LASTEXITCODE
        Write-RemediationLog "gpupdate returned exit code $($script:GpupdateExitCode)." -Level 'Info'

        if ($script:GpupdateExitCode -ne 0) {
            throw "gpupdate returned exit code $($script:GpupdateExitCode)."
        }
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message 'Group Policy update forced successfully.' -Level 'SUCCESS'
    }
    else {
