<#
.TITLE
    Detection - Group Policy Refresh Request

.SYNOPSIS
    Always returns non-compliant so the paired remediation forces a Group Policy refresh.
...
.DESCRIPTION
    ...
    always invokes the paired remediation that runs gpupdate /force.
...
.TAGS
    Remediation,Detection,GroupPolicy,AlwaysTrigger
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Invoke-GPUpdate\Invoke-GPUpdate-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    $reasons.Add('Always-trigger detector configured - a Group Policy refresh is requested on every run')
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message 'Compliant - no Group Policy refresh requested' -Level 'SUCCESS'
    }
