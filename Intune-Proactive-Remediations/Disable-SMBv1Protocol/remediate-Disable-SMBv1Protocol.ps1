...
.SYNOPSIS
    Disables the SMBv1 server protocol so the device no longer serves legacy SMB shares.

.DESCRIPTION
    Paired remediation for Disable-SMBv1Protocol. Runs only when
    detect-Disable-SMBv1Protocol.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) SMB configuration fix with failure tracking, (3) post-remediation
...
.TAGS
    Remediation,Action,SMBv1,Security
...
.PERMISSIONS
    None (local SYSTEM context) - changes the SMB server configuration.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Disable-SMBv1Protocol\Disable-SMBv1Protocol-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The SMB server configuration must be readable before it can be modified.
        $null = Get-SmbServerConfiguration -ErrorAction Stop
...
function Test-FixApplied {
    try {
        # Re-check the same setting the detector read. Return $true only when
        # the observed state now matches the compliant definition.
        $afterValue = (Get-SmbServerConfiguration -ErrorAction Stop).EnableSMB1Protocol
        return (-not $afterValue)
    }
    catch {
        Write-RemediationLog "Verification could not read the SMB server configuration: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Write-RemediationLog "Disabling the SMBv1 server protocol (EnableSMB1Protocol=0)" -Level 'Info'

    Invoke-FixTarget -TargetName "EnableSMB1Protocol=0" -Fix {
