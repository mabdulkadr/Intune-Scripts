<#
.TITLE
    Remediation - Enable Multi-Session Concurrent Logons

.SYNOPSIS
    Removes single-session restrictions so Windows allows concurrent user sessions.

.DESCRIPTION
    Paired remediation for Enable-MultiSession. Runs only when
    detect-Enable-MultiSession.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) registry fixes with per-target failure tracking,
    ...
    1. HKLM:\System\CurrentControlSet\Control\Terminal Server
       - fSingleSessionPerUser = 0 (created when missing)
    2. Optional acNam Credential Provider (only when the provider path exists):
       HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Authentication\Credential Providers\{B12744B8-5BB7-463a-B85E-BB7627E73002}
       - EnforceSingleLogon = 0
...
.TAGS
    Remediation,Action,MultiSession,TerminalServer
...
.PERMISSIONS
    None (local SYSTEM context) - sets registry DWORD values only.
...
.CHANGELOG
    ...
    - acNam provider is now optional-skip instead of a hard failure path
    - Aligned script-scope variables to $script:PascalCase (NAMING LAW) to match collection standard
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Enable-MultiSession\Enable-MultiSession-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        if (-not (Test-Path -LiteralPath $TerminalServerPath)) {
            throw "Required registry path was not found: $TerminalServerPath"
        }
...
function Test-FixApplied {
    try {
        # Re-check the same values the detector reads. Return $true only when
        # the observed state now matches the compliant definition. A value set
        # to 0, or absent (Windows default), means multi-session is allowed.
        $terminal = (Get-ItemProperty -LiteralPath $TerminalServerPath -ErrorAction SilentlyContinue).$TerminalServerName
        if ($terminal -eq 1) {
            return $false
        }

        if (Test-Path -LiteralPath $CredentialProvPath) {
            $provider = (Get-ItemProperty -LiteralPath $CredentialProvPath -ErrorAction SilentlyContinue).$CredentialProvName
            if ($provider -eq 1) {
                return $false
            }
        }
        ...
        Write-RemediationLog "Verification could not read the target registry values: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    $currentTerminal = (Get-ItemProperty -LiteralPath $TerminalServerPath -ErrorAction Stop).$TerminalServerName
    Write-RemediationLog "Current value [$TerminalServerPath\$TerminalServerName] = $currentTerminal" -Level 'Info'

    Invoke-FixTarget -TargetName "$TerminalServerName=$TerminalTargetValue" -Fix {
        Set-RegistryDwordValue -Path $TerminalServerPath -Name $TerminalServerName -Value $TerminalTargetValue
    }
    Write-RemediationLog "Set $TerminalServerName to $TerminalTargetValue." -Level 'Info'

    if (Test-Path -LiteralPath $CredentialProvPath) {
        $targetCount++
        $currentProvider = (Get-ItemProperty -LiteralPath $CredentialProvPath -ErrorAction Stop).$CredentialProvName
        Write-RemediationLog "Current value [$CredentialProvPath\$CredentialProvName] = $currentProvider" -Level 'Info'

        Invoke-FixTarget -TargetName "$CredentialProvName=$CredentialTargetVal" -Fix {
            Set-RegistryDwordValue -Path $CredentialProvPath -Name $CredentialProvName -Value $CredentialTargetVal
        }
        Write-RemediationLog "Set $CredentialProvName to $CredentialTargetVal." -Level 'Info'
    }
    else {
        Write-RemediationLog "acNam Credential Provider path not found - skipping vendor-specific remediation." -Level 'Info'
    }
