...
$ScriptName   = 'Reset-WindowsUpdateComponents--Detect.ps1'
$SolutionName = 'Reset-WindowsUpdateComponents'
$ScriptMode   = 'Detection'

$SystemDrive = if ($env:SystemDrive) { $env:SystemDrive.TrimEnd('\') } else { [System.IO.Path]::GetPathRoot($env:SystemRoot).TrimEnd('\') }
$LogRoot     = Join-Path $SystemDrive "IntuneLogs\$SolutionName"
$LogFile     = Join-Path $LogRoot 'Reset-WindowsUpdateComponents--Detect.txt'
$BannerLine  = '=' * 78
...
try {
    Initialize-Log
    Write-Banner
    Finish-Script -Message 'Non-Compliant: Windows Update component reset is always required.' -ExitCode 1 -Level 'WARNING'
}
catch {
