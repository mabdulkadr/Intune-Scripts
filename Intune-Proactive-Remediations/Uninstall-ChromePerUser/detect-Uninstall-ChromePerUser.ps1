<#
.TITLE
    Detection - Per-User Chrome Installed

.SYNOPSIS
    Checks whether Google Chrome is registered as a per-user installed application.

.DESCRIPTION
    Evaluates uninstall entries under the current user's registry hive and
    reports every entry whose normalized DisplayName or DisplayName_Localized
    matches the blacklist (Google Chrome). A per-user Chrome installation makes
    the device non-compliant so the paired remediation can remove it silently.
...
.TAGS
    Remediation,Detection,Chrome,Uninstall
...
.PERMISSIONS
    None (local SYSTEM context) - reads HKCU uninstall registry entries of the signed-in user.
...
.NOTES
    - Requires the signed-in user context: HKCU points to the profile of the
      account running the script, so deploy with "Run this script using
      logged-on credentials: Yes" in Intune.
    - Keep detection under 30 seconds: one registry key enumeration only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Uninstall-ChromePerUser\Uninstall-ChromePerUser-Detection.txt
#>
...
function Get-PerUserBlacklistEntries {
    if (-not (Test-Path -LiteralPath $UninstallRegPath)) {
        # Missing Uninstall key means no per-user applications are registered.
        return @()
    }
...
function Test-ComplianceState {
    ...
    try {
        $matchedEntries = @(Get-PerUserBlacklistEntries)

        foreach ($entry in $matchedEntries) {
            $reasons.Add("Per-user Google Chrome uninstall entry found under $($entry.RegistryPath)")
        }
    }
    catch {
        throw "Failed to scan user uninstall entries: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Per-user Google Chrome was not detected" -Level 'SUCCESS'
    }
