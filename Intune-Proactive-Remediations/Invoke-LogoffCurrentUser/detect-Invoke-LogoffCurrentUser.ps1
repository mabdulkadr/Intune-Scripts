<#
.TITLE
    Detection - Current User Logoff Request

.SYNOPSIS
    Always returns non-compliant so the paired remediation logs off the current user.

.DESCRIPTION
    This detector intentionally does not evaluate device state. It is an
    always-trigger detector: it reports one condition on every run so Intune
    always invokes the paired remediation that warns the user and signs out the
...
.TAGS
    Remediation,Detection,Session,AlwaysTrigger
...
.PERMISSIONS
    None (local SYSTEM context) - performs no system evaluation.
...
.CHANGELOG
    ...
    1.2
    - Always-trigger detection for the logoff remediation pair
    1.0.0
    - Initial release
...
.EXAMPLE
    ...
    Returns exit 1 on every run so Intune always triggers the paired remediation.
...
.NOTES
    ...
    - Always-run intent: preserved by design; exit 0 is unreachable.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Invoke-LogoffCurrentUser\Invoke-LogoffCurrentUser-Detection.txt
#>
...
# Always-trigger detector: intentionally non-empty on every run.
function Test-ComplianceState {
    $reasons = [System.Collections.Generic.List[string]]::new()

    $reasons.Add('Always-trigger detector configured - a current-user logoff is requested on every run')
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message 'Compliant - no user logoff requested' -Level 'SUCCESS'
    }
