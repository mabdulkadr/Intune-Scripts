<#
.TITLE
    Remediation - Immediate Forced Restart

.SYNOPSIS
    Forces an immediate restart without any warning or delay.

.DESCRIPTION
    Immediate/aggressive-flavor remediation paired with
    detect-Restart-ComputerForced.ps1. Issues Restart-Computer -Force at once
    and does not depend on the delayed warning workflow: no status file is
    read, no balloon tips are shown, and no grace period is given. The
    considerate flavor is remediate-Restart-ComputerForced.ps1, which warns the
    user, waits 30 minutes by default plus a final minute, and only restarts
    when the detector's status file says 'Restart required'.

    Exit contract:
    Exit 0 = success (restart command issued)
    Exit 1 = failure (verification failed after applying)
    Exit 2 = script error

.TAGS
    Remediation,Action,Reboot,ImmediateRestart
...
.PERMISSIONS
    None (local SYSTEM context) - forces an immediate operating system restart.
...
.EXAMPLE
    ...
    Forces an immediate restart with no warnings; intended for aggressive maintenance windows.
...
.NOTES
    ...
    - A forced immediate restart discards unsaved work - assign only to maintenance scopes.
    - Logs: <SystemDrive>\IntuneLogs\Restart-ComputerForcedNow\Restart-ComputerForcedNow-Remediation.txt
#>
...
$SolutionName = 'Restart-ComputerForcedNow'
...
function Test-RemediationPrerequisites {
    try {
        ...
        Write-RemediationLog "Immediate restart prerequisites satisfied" -Level 'Info'
...
try {
    ...
    Invoke-FixTarget -TargetName 'ImmediateForcedRestart' -Fix {
        # No status-file dependency, no balloon tips, no delay: this is the
        # immediate/aggressive flavor of the paired delayed remediation.
        Write-RemediationLog "Forcing system restart immediately" -Level 'Warning'
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Immediate forced restart command issued successfully" -Level 'SUCCESS'
    }
    else {
