...
.DESCRIPTION
    This detection script does not evaluate device state. It always exits with
    code 1 so Intune invokes the paired remediation script every time.
...
$ScriptName   = 'Stop-ComputerImmediate--Detect.ps1'
$SolutionName = 'Stop-ComputerImmediate'
$ScriptMode   = 'Detection'

$SystemDrive = if ($env:SystemDrive) { $env:SystemDrive.TrimEnd('\') } else { [System.IO.Path]::GetPathRoot($env:SystemRoot).TrimEnd('\') }
$LogRoot     = Join-Path $SystemDrive "IntuneLogs\$SolutionName"
$LogFile     = Join-Path $LogRoot 'Stop-ComputerImmediate--Detect.txt'
$BannerLine  = '=' * 78
...
Finish-Script -ExitCode 1 -Level 'WARNING' -Message 'Non-Compliant: Remediation is intentionally triggered on every run.'
