<#
.TITLE
    Detection - Scheduled Restart Trigger

.SYNOPSIS
    Always returns a non-zero detection result so the paired remediation script runs.

.DESCRIPTION
    Always-trigger package: this detection intentionally does not evaluate any
    device state and reports non-compliant on every run so Intune invokes the
    paired remediation, which shows a restart notice and schedules a restart.
    This design is preserved exactly as documented in the legacy package. This
...
.TAGS
    Remediation,Detection,Restart,Maintenance
...
.PERMISSIONS
    None (local SYSTEM context) - evaluates nothing and changes nothing.
...
.EXAMPLE
    ...
    Always returns exit 1 so the paired remediation runs every cycle.
...
.NOTES
    ...
    - Keep detection under 30 seconds: no device queries at all.
    - WARNING: the paired remediation schedules a machine restart - assign carefully.
    - Logs: <SystemDrive>\IntuneLogs\Stop-ComputerImmediate\Stop-ComputerImmediate-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    # Always-trigger package preserved: non-compliant by design on every run so
    # Intune invokes the paired remediation each cycle.
    $reasons.Add("Non-compliant by design - remediation is intentionally triggered on every run")
