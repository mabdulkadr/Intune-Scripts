<#
.TITLE
    Remediation - Clear Downloads Folders for All Users

.SYNOPSIS
    Deletes all content under every local user profile's Downloads folder.

.DESCRIPTION
    Paired remediation for Clear-DownloadsFolderAllUsers. Runs whenever Intune executes it because
    the paired detector intentionally always reports non-compliant. Enumerates local user profiles
    under C:\Users, locates each existing Downloads folder, and removes all child content
    recursively. Performs: (1) pre-remediation validation, (2) per-folder deletion with failure
...
.TAGS
    Remediation,Action,Maintenance,Downloads,Cleanup
...
.PERMISSIONS
    None (local SYSTEM context) - deletes file content under every user profile's Downloads folder.
...
.EXAMPLE
    ...
    Clears every user Downloads folder and verifies the result; exits 0 on success.
...
.NOTES
    ...
    - Idempotent: already-empty folders are skipped and safe to process repeatedly.
    - Logs: <SystemDrive>\IntuneLogs\Clear-DownloadsFolderAllUsers\Clear-DownloadsFolderAllUsers-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The user profiles root must exist before any Downloads folder can be cleared.
        if (-not (Test-Path -LiteralPath $UserProfilesRoot -PathType Container)) {
            throw "Required directory not found: $UserProfilesRoot"
        }
...
function Test-FixApplied {
    try {
        # Re-enumerate every discovered Downloads folder; compliant only when no
        # child content remains in any of them.
        foreach ($downloadsFolder in $script:DownloadsFolders) {
            if (-not (Test-Path -LiteralPath $downloadsFolder)) {
                continue
            }

            $remaining = @(Get-ChildItem -LiteralPath $downloadsFolder -Force -ErrorAction SilentlyContinue)
            if ($remaining.Count -gt 0) {
                Write-RemediationLog "$($remaining.Count) item(s) remain in: $downloadsFolder" -Level 'Warning'
                return $false
            }
        }
        return $true
        ...
        Write-RemediationLog "Verification could not enumerate Downloads folders: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount        = 0
    $processedCount     = 0
    $script:DownloadsFolders = @()

    Write-RemediationLog "Executing remediation actions..." -Level 'Info'
    Write-RemediationLog "Scanning user profiles under: $UserProfilesRoot" -Level 'Info'

    $profileDirectories = @(Get-ChildItem -LiteralPath $UserProfilesRoot -Directory -ErrorAction Stop)
    foreach ($profileDirectory in $profileDirectories) {
        $downloadsPath = Join-Path $profileDirectory.FullName 'Downloads'
        if (Test-Path -LiteralPath $downloadsPath) {
            $script:DownloadsFolders += $downloadsPath
        }
    }

    foreach ($downloadsFolder in $script:DownloadsFolders) {
        $items = @(Get-ChildItem -LiteralPath $downloadsFolder -Force -ErrorAction SilentlyContinue)
        if (@($items).Count -eq 0) {
            Write-RemediationLog "Skipping empty Downloads folder: $downloadsFolder" -Level 'Info'
            continue
        }

        $targetCount++
        Write-RemediationLog "Clearing Downloads folder: $downloadsFolder" -Level 'Info'

        $fixed = Invoke-FixTarget -TargetName $downloadsFolder -Fix {
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Downloads cleanup completed for $processedCount user folder(s)." -Level 'SUCCESS'
    }
    else {
