<#
.TITLE
    Detection - Dell SupportAssist Not Installed

.SYNOPSIS
    Verifies that Dell SupportAssist is not present in the standard uninstall registry locations.

.DESCRIPTION
    Searches the 64-bit and 32-bit HKLM uninstall registry keys for an application
    named exactly 'Dell SupportAssist'. When the application is found, the reason
    string includes its UninstallString so remediation can reuse the same removal
    method. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Dell,SupportAssist
...
.PERMISSIONS
    None (local SYSTEM context) - reads standard HKLM uninstall registry keys.
...
.NOTES
    ...
    - Keep detection under 30 seconds: two registry uninstall key queries only.
    - Read-only by definition; detection never uninstalls anything.
    - Logs: <SystemDrive>\IntuneLogs\Uninstall-DellSupportAssistApp\Uninstall-DellSupportAssistApp-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        Write-Log -Message 'Searching uninstall registry locations for Dell SupportAssist' -Level 'DEBUG'
        ...
        if ($app) {
            # Include the uninstall string so remediation reuses the same removal method.
            $reasons.Add("Dell SupportAssist found - UninstallString: $($app.UninstallString)")
        }
        else {
            Write-Log -Message 'Dell SupportAssist was not found in the uninstall registry' -Level 'DEBUG'
        }
    }
    catch {
        throw "Failed to query the uninstall registry locations: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Dell SupportAssist is not installed" -Level 'SUCCESS'
    }
