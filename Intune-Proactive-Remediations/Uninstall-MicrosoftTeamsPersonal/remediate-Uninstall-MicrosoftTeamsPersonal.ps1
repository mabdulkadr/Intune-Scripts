<#
.TITLE
    Remediation - Remove Microsoft Teams Personal

.SYNOPSIS
    Removes the MicrosoftTeams AppX package (personal Teams client) from all users.

.DESCRIPTION
    Paired remediation for Uninstall-MicrosoftTeamsPersonal. Runs only when
    detect-Uninstall-MicrosoftTeamsPersonal.ps1 returns exit 1. Gets the AppX
    package named MicrosoftTeams across all users and removes each discovered
    package with Remove-AppxPackage (using -AllUsers when the cmdlet supports
    it). It targets the Store/AppX form of Teams only; the new Teams client
    (MSTeams) is not touched. Performs: (1) pre-remediation validation, (2)
    per-package removal with failure tracking, (3) post-remediation
...
.PERMISSIONS
    None (local SYSTEM context) - removes AppX packages for all users.
...
.EXAMPLE
    ...
    Removes every discovered MicrosoftTeams AppX instance and verifies; exits 0 on verified success.
...
.NOTES
    - Runs in SYSTEM context via Intune Proactive Remediations (-AllUsers requires elevation).
    - Idempotent: safe to run repeatedly; verify-before-and-after.
    - Logs: <SystemDrive>\IntuneLogs\Uninstall-MicrosoftTeamsPersonal\Uninstall-MicrosoftTeamsPersonal-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # Remove-AppxPackage must be available before any package can be removed.
        ...
        $script:TeamsPackages = @(Get-AppxPackage -Name $TargetPackageName -AllUsers -ErrorAction Stop)

        if ($script:TeamsPackages.Count -eq 0) {
            Write-RemediationLog "MicrosoftTeams AppX package was not found. Nothing to remediate." -Level 'Info'
        }
        else {
            Write-RemediationLog "Found $($script:TeamsPackages.Count) MicrosoftTeams AppX package instance(s)" -Level 'Info'
        }
...
# Removes one package, preferring the AllUsers parameter when supported.
...
function Test-FixApplied {
    try {
        ...
        # MicrosoftTeams AppX instance remains.
        $remaining = @(Get-AppxPackage -Name $TargetPackageName -AllUsers -ErrorAction Stop)
        return ($remaining.Count -eq 0)
    }
    catch {
        Write-RemediationLog "Verification could not enumerate AppX packages: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount        = $script:TeamsPackages.Count

    Write-RemediationLog "Executing remediation actions..." -Level 'Info'

    foreach ($package in $script:TeamsPackages) {
        Write-RemediationLog "Removing AppX package: $($package.PackageFullName)" -Level 'Info'

        Invoke-FixTarget -TargetName $package.PackageFullName -Fix {
            Remove-TeamsPersonalPackage -PackageFullName $package.PackageFullName
            Write-RemediationLog "Removed AppX package: $($package.PackageFullName)" -Level 'Info'
        }
    }
