<#
.TITLE
    Remediation - Restart Windows Search Service

.SYNOPSIS
    Restarts the Windows Search service and verifies that it returns to a running state.

.DESCRIPTION
    Paired remediation for Restart-WindowsSearch. Runs only when
    detect-Restart-WindowsSearch.ps1 returns exit 1. Confirms the WSearch
    service exists, restarts it, then reads the service again to confirm the
    post-restart state. Performs: (1) pre-remediation validation, (2) service
    restart with failure tracking, (3) post-remediation verification,
...
.TAGS
    Remediation,Action,Services,WindowsSearch
...
.EXAMPLE
    ...
    Restarts Windows Search and verifies it is running; exits 0 on success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Restart-WindowsSearch\Restart-WindowsSearch-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The WSearch service must exist before Restart-Service touches it.
        $service = Get-Service -Name $servicename -ErrorAction SilentlyContinue
        if (-not $service) {
            throw "Windows Search service was not found: $servicename"
        }
...
function Test-FixApplied {
    try {
        # Re-read the same object type the detector used. Return $true only when
        # the observed state now matches the desired state (Running).
        $service = Get-Service -Name $servicename -ErrorAction SilentlyContinue
        if (-not $service) {
            Write-RemediationLog "Verification could not find the Windows Search service" -Level 'Error'
            return $false
        }
...
try {
    ...
    # Preserved legacy outcome: a missing service is a failed remediation, not a crash.
    ...
    if (-not $serviceInfo) {
        ...
        Finish-Script -ExitCode 1 -Message 'Windows Search service was not found.' -Level 'ERROR'
    }
    ...
    $fixApplied = Invoke-FixTarget -TargetName "Restart service: $servicename" -Fix {
        Write-RemediationLog "Restarting Windows Search service: $servicename" -Level 'Info'

        Restart-Service -Name $servicename -Force -ErrorAction Stop

        Write-RemediationLog 'Restart-Service completed for Windows Search.' -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Windows Search service restart completed successfully. Current state: Running" -Level 'SUCCESS'
    }
    else {
        $script:RemediationResult.Status = "Failed"
        Write-Output "Remediation finished but verification failed"
        Write-Output ($remediationResult | ConvertTo-Json -Depth 6 -Compress)
        Finish-Script -ExitCode 1 -Message "Windows Search service restart completed, but the service is not running" -Level 'ERROR'
    }
}
catch {
