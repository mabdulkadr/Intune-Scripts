<#
.TITLE
    Detection - Windows Search Service Presence

.SYNOPSIS
    Checks whether the Windows Search service exists or appears to be running.

.DESCRIPTION
    Evaluates the WSearch service and preserves the original permissive detection
    behavior: the script considers Windows Search compliant when its internal
    counter is non-zero, which means the service merely existing is enough to
    satisfy the check even when it is not running. Only a service that cannot be
    found is reported non-compliant. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Services,WindowsSearch
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Restart-WindowsSearch\Restart-WindowsSearch-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        # Preserved permissive template behavior: any non-zero counter counts as compliant.
        $checkarray   = 0
        $serviceExist = Get-Service -Name $servicename -ErrorAction SilentlyContinue

        if ($serviceExist) {
            $checkarray++
            Write-Log -Message "Windows Search service exists: $servicename" -Level 'SUCCESS'
            ...
            if ($serviceExist.Status -eq 'Running') {
                ...
                Write-Log -Message "Windows Search service is not running. Current state: $($serviceExist.Status)" -Level 'WARNING'
            }
        }
        else {
            Write-Log -Message 'Windows Search service was not found.' -Level 'WARNING'
            $reasons.Add('Windows Search service was not found')
        }
    }
    catch {
        throw "Failed to evaluate the Windows Search service state: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - the template considers Windows Search available or running" -Level 'SUCCESS'
    }
    ...
    Finish-Script -ExitCode 1 -Message "Non-compliant - The template considers Windows Search non-compliant." -Level 'WARNING'
}
catch {
