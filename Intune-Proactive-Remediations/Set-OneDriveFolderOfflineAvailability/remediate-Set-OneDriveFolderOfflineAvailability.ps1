<#
.TITLE
    Remediation - Pin OneDrive Folder for Offline Use

.SYNOPSIS
    Pins the configured OneDrive folder and its contents for offline use.

.DESCRIPTION
    Paired remediation for Set-OneDriveFolderOfflineAvailability. Runs only when
    detect-Set-OneDriveFolderOfflineAvailability.ps1 returns exit 1. Performs:
    (1) pre-remediation validation, (2) attrib.exe pinning with failure tracking,
    (3) post-remediation verification, (4) structured JSON result output for
    Intune diagnostics. The original mechanism is preserved: attrib.exe -U +P /s /d
    on the root folder, then -U +P on each child item recursively.
...
.TAGS
    Remediation,Action,OneDrive,Files
...
.PERMISSIONS
    None (local SYSTEM context) - modifies file attributes of the signed-in user's OneDrive folder.
...
.EXAMPLE
    ...
    Pins the OneDrive folder tree and verifies the pin state; exits 0 on verified success.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Set-OneDriveFolderOfflineAvailability\Set-OneDriveFolderOfflineAvailability-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The target folder must exist before attrib.exe can pin it.
        $targetPath = Get-OneDriveTargetPath
        if (-not (Test-Path -LiteralPath $targetPath)) {
            throw "Required OneDrive folder not found: $targetPath"
        }
...
function Test-FixApplied {
    try {
        # Re-check the same attribute pattern the detector evaluated. Return $true
        # only when the root folder now reports the pinned R/P attributes.
        $targetPath = Get-OneDriveTargetPath
        $attribOutput = (& attrib.exe $targetPath 2>&1 | Out-String).Trim()
        $normalizedOutput = ($attribOutput -replace '\s+', '')
        return ($normalizedOutput -match 'RP')
    }
    catch {
        Write-RemediationLog "Verification could not read folder attributes: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "Pin OneDrive folder $($CompanyName)\$($ODFolder)" -Fix {
        $targetPath = Get-OneDriveTargetPath
        Write-RemediationLog "Target path: $targetPath" -Level 'Info'

        # Original flags preserved: pin the whole root tree first...
        & attrib.exe $targetPath -U +P /s /d | Out-Null
        Write-RemediationLog 'Pinned the root OneDrive folder tree for offline use.' -Level 'Info'

        # ...then apply -U +P to each child item recursively.
        $childItems = Get-ChildItem -LiteralPath $targetPath -Recurse -Force -ErrorAction Stop
        foreach ($item in $childItems) {
            & attrib.exe $item.FullName -U +P | Out-Null
        }

        Write-RemediationLog ("Applied offline availability attributes to child items: {0}" -f $childItems.Count) -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "The OneDrive folder was pinned for offline availability successfully" -Level 'SUCCESS'
    }
    else {
