<#
.TITLE
    Remediation - Enable Defender Cloud Protection

.SYNOPSIS
    Enables Microsoft Defender cloud-delivered protection settings.

.DESCRIPTION
    Paired remediation for Get-DefenderCloudProtectionStatus. Runs only when
    detect-Get-DefenderCloudProtectionStatus.ps1 returns exit 1. Performs:
    (1) pre-remediation validation, (2) Defender preference fixes with failure
    tracking (MAPSReporting=Advanced, SubmitSamplesConsent=SendAllSamples),
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-DefenderCloudProtectionStatus\Get-DefenderCloudProtectionStatus-Remediation.txt
#>
...
function Test-FixApplied {
    try {
        # Re-check the same values the detector read. Return $true only when
        # the observed state now matches the compliant definition.
        $preferences = Get-MpPreference -ErrorAction Stop
        return (($preferences.MAPSReporting -eq $ExpectedMapsReporting) -and ($preferences.SubmitSamplesConsent -eq $ExpectedSamplesConsent))
    }
    catch {
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName "MAPSReporting=Advanced" -Fix {
        Set-MpPreference -MAPSReporting Advanced -ErrorAction Stop
        Write-RemediationLog "MAPSReporting was set to Advanced" -Level 'Info'
    }

    $targetCount++
    Invoke-FixTarget -TargetName "SubmitSamplesConsent=SendAllSamples" -Fix {
        Set-MpPreference -SubmitSamplesConsent SendAllSamples -ErrorAction Stop
        Write-RemediationLog "SubmitSamplesConsent was set to SendAllSamples" -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Defender cloud-delivered protection settings were applied successfully ($Version Remediated)" -Level 'SUCCESS'
    }
    else {
