...
$ScriptName   = 'Repair-Disk--Detect.ps1'
$SolutionName = 'Repair-Disk'
$ScriptMode   = 'Detection'

$SystemDrive = if ($env:SystemDrive) { $env:SystemDrive.TrimEnd('\') } else { [System.IO.Path]::GetPathRoot($env:SystemRoot).TrimEnd('\') }
$LogRoot     = Join-Path $SystemDrive "IntuneLogs\$SolutionName"
$LogFile     = Join-Path $LogRoot 'Repair-Disk--Detect.txt'
$BannerLine  = '=' * 78
...
Finish-Script -ExitCode 1 -Level 'WARNING' -Message 'Detection is configured to always trigger disk repair remediation.' -OutputMessage 'Non-Compliant: Disk repair is required.'
