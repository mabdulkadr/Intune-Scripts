<#
.TITLE
    Remediation - Clear Current User Proxy Settings

.SYNOPSIS
    Disables the current user's WinINET proxy and clears the server string.

.DESCRIPTION
    Paired remediation for Remove-SystemProxySettings. Runs only when
    detect-Remove-SystemProxySettings.ps1 returns exit 1. Performs: (1) pre-remediation
    validation that the Internet Settings key exists, (2) registry writes with
    failure tracking (ProxyEnable = 0, ProxyServer cleared), (3) post-remediation
    verification that both values match the cleared state, (4) structured JSON
...
.TAGS
    Remediation,Action,Proxy,WinINET,Networking
...
.PERMISSIONS
    None (local SYSTEM context) - writes two HKCU registry values.
...
.CHANGELOG
    ...
    1.2
    - ProxyEnable reset plus ProxyServer clearing with read-back verification
    1.0.0
    - Initial release
...
.EXAMPLE
    ...
    Clears the current user proxy settings and verifies; exits 0 on verified success.
...
.NOTES
    ...
    - Verification semantics: the values are re-read immediately after the write;
      applications may cache proxy state until they restart.
    - Logs: <SystemDrive>\IntuneLogs\Remove-SystemProxySettings\Remove-SystemProxySettings-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The parent key must exist before Set-ItemProperty can write to it.
        if (-not (Test-Path -LiteralPath $RegistryPath)) {
            throw "Required registry key not found: $RegistryPath"
        }

        # Record the pre-state for the audit trail.
        $internetSettings = Get-ItemProperty -LiteralPath $RegistryPath -ErrorAction Stop
        Write-RemediationLog "Current ProxyEnable: $([int]($internetSettings.ProxyEnable))" -Level 'Info'
...
function Test-FixApplied {
    try {
        ...
        return ($proxyEnabled -eq 0 -and [string]::IsNullOrWhiteSpace($proxyServer))
    }
    catch {
        Write-RemediationLog "Verification could not read proxy settings: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    $targetCount++
    Invoke-FixTarget -TargetName 'Current user WinINET proxy settings' -Fix {
        # Original legacy actions preserved exactly: ProxyEnable to 0, ProxyServer cleared.
        Set-ItemProperty -LiteralPath $RegistryPath -Name 'ProxyEnable' -Value 0 -ErrorAction Stop
        Set-ItemProperty -LiteralPath $RegistryPath -Name 'ProxyServer' -Value '' -ErrorAction Stop
        Write-RemediationLog 'ProxyEnable set to 0 and ProxyServer cleared.' -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message 'Current user proxy settings were removed successfully.' -Level 'SUCCESS'
    }
    else {
