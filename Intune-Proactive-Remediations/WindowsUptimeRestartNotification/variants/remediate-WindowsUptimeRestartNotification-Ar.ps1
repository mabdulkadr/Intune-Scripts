<#
.TITLE
    Remediation - Windows Restart Notification (Arabic)

.SYNOPSIS
    Shows an Arabic WPF restart prompt with immediate, 1-hour, and 2-hour options.
...
.DESCRIPTION
    ...
    borderless, right-to-left WPF dialog in Arabic telling the user a restart is
    required (pending servicing reboot and/or uptime threshold reached). The user
    can restart now, schedule in 1 hour or 2 hours, minimize, or dismiss. A forced
    scheduled restart is optional via $ForceRestartWhenPending.

    LANGUAGE WAIVER (Law 11): all code, comments, headers, and log lines are English;
    ONLY the end-user-visible dialog strings remain Arabic by explicit owner request.
    Save this file as UTF-8 with BOM so PowerShell 5.1 renders Arabic correctly.
...
.EXAMPLE
    ...
    Shows the Arabic restart dialog when a reboot is pending or uptime exceeded.
...
.NOTES
    ...
    - Save as UTF-8 with BOM for Arabic in Windows PowerShell 5.1.
...
# User-facing strings are intentionally Arabic (owner-approved Law 11 waiver).
# ============================================================================
...
function Get-PendingRebootInfo {
    ...
    if (Test-Path -LiteralPath 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired') {
        # Display string is user-facing (Arabic waiver).
...
try {
    ...
    # --- Build user-facing message (Arabic waiver: displayed strings stay Arabic) ---
    $txtMessageTitle = 'يلزم إعادة تشغيل الجهاز'
    $sections = @()
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Window.Resources>
        ...
        <!-- Canonical Tailwind Slate token names (Identity Lock) -->
