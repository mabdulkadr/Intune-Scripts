<#
.TITLE
    Remediation - Disable Fast Startup

.SYNOPSIS
    Sets HiberbootEnabled to 0 so Windows always performs a full startup.

.DESCRIPTION
    Paired remediation for Disable-FastStartup. Runs only when
    detect-Disable-FastStartup.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) registry fix with failure tracking, (3) post-remediation
...
.TAGS
    Remediation,Action,Power,FastStartup
...
.PERMISSIONS
    None (local SYSTEM context) - writes one HKLM registry value.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Disable-FastStartup\Disable-FastStartup-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The parent registry key must exist before Set-ItemProperty can write to it.
...
function Test-FixApplied {
    try {
        ...
        $afterValue = Get-ItemPropertyValue -LiteralPath $RegistryPath -Name $RegistryName -ErrorAction Stop
        return ($afterValue -eq $DesiredValue)
    }
    catch {
...
try {
    ...
    $beforeValue = $null
    try {
        $beforeValue = Get-ItemPropertyValue -LiteralPath $RegistryPath -Name $RegistryName -ErrorAction Stop
    }
    catch [System.Management.Automation.ItemNotFoundException] {
        # Value absent is an expected pre-state - it gets created by the fix below.
        Write-RemediationLog "$RegistryName not currently set - it will be created" -Level 'Info'
    }
    if ($null -ne $beforeValue) {
        Write-RemediationLog "Current $RegistryName value: $beforeValue" -Level 'Info'
    }

    Invoke-FixTarget -TargetName "$RegistryName=0" -Fix {
        Set-ItemProperty -LiteralPath $RegistryPath -Name $RegistryName -Value $DesiredValue -Type DWord -Force -ErrorAction Stop | Out-Null
    }
