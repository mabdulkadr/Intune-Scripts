<#
.TITLE
    Detection - Fast Startup Disabled

.SYNOPSIS
    Verifies that Windows Fast Startup (Hiberboot) is disabled via the Power registry key.

.DESCRIPTION
    Evaluates whether Fast Startup is disabled by reading HiberbootEnabled from
    HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Power. Fast Startup can
    cause kernel/session state issues on managed devices and blocks clean restarts,
    so enterprise baselines require it disabled. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,Power,FastStartup
...
.PERMISSIONS
    None (local SYSTEM context) - reads one HKLM registry value.
...
.NOTES
    ...
    - Keep detection under 30 seconds: single registry read only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Disable-FastStartup\Disable-FastStartup-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        $currentValue = Get-ItemPropertyValue -LiteralPath $RegistryPath -Name $RegistryName -ErrorAction Stop
        Write-Log -Message "Current $RegistryName value: $currentValue" -Level 'DEBUG'

        if ($currentValue -ne $ExpectedValue) {
            $reasons.Add("Fast Startup is enabled - expected $RegistryName=$ExpectedValue, found $currentValue")
        }
    }
    catch [System.Management.Automation.ItemNotFoundException] {
        $reasons.Add("Registry value missing: $RegistryPath\$RegistryName does not exist")
    }
    catch {
        throw "Failed to read ${RegistryName}: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - Fast Startup is already disabled" -Level 'SUCCESS'
    }
