<#
.TITLE
    Detection - No Blacklisted Applications Installed

.SYNOPSIS
    Verifies that none of the configured blacklisted applications are installed.

.DESCRIPTION
    Searches the standard 64-bit and 32-bit HKLM uninstall registry locations,
    compares discovered display names against the configurable blacklist array
    ($BlacklistApps), and reports every match found. This script NEVER modifies
    the system.
...
.TAGS
    Remediation,Detection,Uninstall,Blacklist
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Uninstall-InstalledApplication\Uninstall-InstalledApplication-Detection.txt
#>
...
# Returns installed blacklist matches; falls back to the localized display name
# when DisplayName itself is empty.
...
function Test-ComplianceState {
    ...
    try {
        Write-Log -Message 'Searching uninstall registry locations for blacklisted applications' -Level 'DEBUG'

        $blacklistedApps = Get-InstalledBlacklistedApps

        foreach ($app in $blacklistedApps) {
            $reasons.Add("Blacklisted application installed: $($app.Name) - UninstallString: $($app.UninstallString)")
        }
    }
    catch {
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - no blacklisted applications are installed" -Level 'SUCCESS'
    }
