...
$ScriptName = 'Invoke-WindowsUpdateScan--Detect.ps1'
...
$LogFile    = Join-Path $LogRoot 'Invoke-WindowsUpdateScan--Detect.txt'
$BannerLine = '=' * 78
...
try {
    ...
    Write-Log -Message ("Current execution policy is '{0}'. No changes are made by this detection script." -f $currentPolicy)
    ...
    if ($matchingUpdates.Count -gt 0) {
        Finish-Script -ExitCode 1 -Level 'WARNING' -Message ("There are {0} pending matching Windows updates." -f $matchingUpdates.Count) -OutputMessage ("There are {0} pending Windows Updates." -f $matchingUpdates.Count)
    }

    Finish-Script -ExitCode 0 -Level 'SUCCESS' -Message 'No pending matching Windows updates were found.' -OutputMessage 'No pending Windows Updates.'
}
catch {
    Finish-Script -ExitCode 2 -Level 'ERROR' -Message ("Detection error: {0}" -f $_.Exception.Message) -OutputMessage ("Error during detection: {0}" -f $_.Exception.Message)
}
