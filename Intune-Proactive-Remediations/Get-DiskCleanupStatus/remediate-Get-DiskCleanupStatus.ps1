<#
.TITLE
    Remediation - Windows Disk Cleanup

.SYNOPSIS
    Runs Windows Disk Cleanup using the cleanup handlers configured in the script.

.DESCRIPTION
    Paired remediation for Get-DiskCleanupStatus. Runs only when
    detect-Get-DiskCleanupStatus.ps1 returns exit 1. Performs: (1) pre-remediation
    validation, (2) StateFlags registry configuration for the selected cleanup
    handlers followed by "CleanMgr.exe /sagerun:1", (3) post-remediation
    verification, (4) structured JSON result output for Intune diagnostics.
    Verification confirms the handler flags persisted and CleanMgr completed;
    the actual space gain depends on the Windows cleanup pass.
...
.TAGS
    Remediation,Action,DiskSpace,Maintenance
...
.PERMISSIONS
    None (local SYSTEM context) - writes VolumeCaches StateFlags values and runs CleanMgr.exe.
...
.EXAMPLE
    ...
    Configures the handlers, runs Disk Cleanup, and exits 0 when verified.
...
.NOTES
    ...
    - Logs: <SystemDrive>\IntuneLogs\Get-DiskCleanupStatus\Get-DiskCleanupStatus-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The handler parent key must exist before any StateFlags value is written.
        if (-not (Test-Path -LiteralPath $VolumeCachesPath)) {
            throw "Required registry key not found: $VolumeCachesPath"
        }

        # CleanMgr.exe must be present for the /sagerun pass to work.
...
function Test-FixApplied {
    try {
        # Every handler that exists must carry the requested StateFlags value.
        foreach ($cleanupType in $CleanupTypeSelection) {
            ...
            $flagValue = Get-ItemPropertyValue -LiteralPath $handlerPath -Name $StateFlagName -ErrorAction Stop
            if ($flagValue -ne $StateFlagValue) {
                Write-RemediationLog "Verification failed for handler ${cleanupType}: $StateFlagName=$flagValue" -Level 'Error'
                return $false
            }
        }
        ...
        Write-RemediationLog "Verification could not read a $StateFlagName value that should exist: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
    catch {
        Write-RemediationLog "Verification could not read the StateFlags configuration: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    Write-RemediationLog "Configuring selected Disk Cleanup handlers..." -Level 'Info'

    foreach ($cleanupType in $CleanupTypeSelection) {
        $handlerPath = Join-Path $VolumeCachesPath $cleanupType

        if (-not (Test-Path -LiteralPath $handlerPath)) {
            # Handler absent on this image is an expected pre-state - skip it.
            Write-RemediationLog "Cleanup handler not found and will be skipped: $cleanupType" -Level 'Warning'
            continue
        }

        $targetCount++
        Invoke-FixTarget -TargetName "Handler $cleanupType" -Fix {
            New-ItemProperty -Path $handlerPath -Name $StateFlagName -Value $StateFlagValue -PropertyType DWord -Force -ErrorAction Stop | Out-Null
            Write-RemediationLog "Enabled cleanup handler: $cleanupType" -Level 'Info'
        }
    }

    $targetCount++
    Invoke-FixTarget -TargetName "Disk Cleanup pass ($CleanMgrArguments)" -Fix {
        $cleanMgrPath = Join-Path $env:SystemRoot 'System32\cleanmgr.exe'
        Write-RemediationLog "Starting Disk Cleanup: $cleanMgrPath $CleanMgrArguments" -Level 'Info'
    ...
    if ($verificationPassed) {
        ...
        $script:RemediationResult.PostCheckStatus += "All configured handlers report $StateFlagName=$StateFlagValue"
        ...
        Finish-Script -ExitCode 0 -Message "Disk Cleanup completed successfully." -Level 'SUCCESS'
    }
    else {
