<#
.TITLE
    Detection - Free Disk Space Status

.SYNOPSIS
    Checks whether free space on drive C: has dropped below the configured threshold.

.DESCRIPTION
    Reads the free space of drive C: from Get-PSDrive and compares it against
    StorageThresholdGB. Low free space degrades update installs, profile loads,
    and overall device health. This script NEVER modifies the system.
...
.TAGS
    Remediation,Detection,DiskSpace,Maintenance
...
.PERMISSIONS
    None (local SYSTEM context) - reads drive free space only.
...
.EXAMPLE
    ...
    Returns exit 0 while free space is above the threshold; exit 1 below it.
...
.NOTES
    ...
    - Keep detection under 30 seconds: single drive query only.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Get-DiskCleanupStatus\Get-DiskCleanupStatus-Detection.txt
#>
...
# Returns the current free space on drive C: in bytes.
...
function Test-ComplianceState {
    ...
    try {
        ...
        Write-Log -Message "Current free space on C:: $freeGB GB" -Level 'DEBUG'

        if ($freeBytes -lt ($StorageThresholdGB * 1GB)) {
            $reasons.Add("Free space is below the ${StorageThresholdGB} GB threshold - currently $freeGB GB available")
        }
    }
    catch {
        throw "Failed to evaluate free space on drive C:: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - free space on drive C: is above the $StorageThresholdGB GB threshold" -Level 'SUCCESS'
    }
