<#
.TITLE
    Detection - Active Directory Secure Channel

.SYNOPSIS
    Verifies that the computer secure channel to the Active Directory domain is healthy.

.DESCRIPTION
    Evaluates whether the device is domain-joined and, when it is, tests the machine
    secure channel with Test-ComputerSecureChannel. A broken secure channel blocks
    domain authentication, Group Policy application, and domain-based Intune
    co-management, so enterprise baselines require it healthy. Devices that are not
    domain-joined are compliant by definition (not applicable). This script NEVER
    modifies the system.
...
.TAGS
    Remediation,Detection,ActiveDirectory,SecureChannel
...
.PERMISSIONS
    None (local SYSTEM context) - runs a read-only secure channel test and one CIM hardware query.
...
.NOTES
    ...
    - Keep detection under 30 seconds: one CIM query plus one secure channel test.
    - Requires line-of-sight to a domain controller for a meaningful result.
    - Idempotent and read-only by definition.
    - Logs: <SystemDrive>\IntuneLogs\Repair-ADSecureChannel\Repair-ADSecureChannel-Detection.txt
#>
...
function Test-ComplianceState {
    ...
    try {
        ...
        # Not domain-joined means the secure channel does not apply - compliant by design.
        if (-not $computerSystem.PartOfDomain) {
            Write-Log -Message 'Device is not domain-joined. Secure channel check is not applicable.' -Level 'DEBUG'
            return @($reasons)
        }

        Write-Log -Message ("Domain: {0}" -f $computerSystem.Domain) -Level 'DEBUG'
        Write-Log -Message 'Device is domain-joined. Testing the computer secure channel.' -Level 'DEBUG'
        ...
        if (-not $secureChannelHealthy) {
            $reasons.Add("Secure channel to domain '$($computerSystem.Domain)' is broken")
        }
    }
        ...
        throw "Failed to test the computer secure channel: $($_.Exception.Message)"
    }
...
try {
    ...
    if ($reasons.Count -eq 0) {
        Finish-Script -ExitCode 0 -Message "Compliant - secure channel is healthy or the device is not domain-joined" -Level 'SUCCESS'
    }
