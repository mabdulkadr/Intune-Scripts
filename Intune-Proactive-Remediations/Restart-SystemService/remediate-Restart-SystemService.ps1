<#
.TITLE
    Remediation - Restart Configured Service

.SYNOPSIS
    Restarts the configured Windows service.

.DESCRIPTION
    Paired remediation for Restart-SystemService. Runs only when
    detect-Restart-SystemService.ps1 returns exit 1. Template script that
    forcibly restarts the service defined in $servicename. Performs:
    (1) pre-remediation validation, (2) service restart with failure tracking,
...
.PERMISSIONS
    None (local SYSTEM context) - restarts one local Windows service.
...
.EXAMPLE
    ...
    Restarts the configured service and verifies it is running; exits 0 on success.
...
.NOTES
    ...
    - TEMPLATE: replace the $servicename placeholder value before production use.
    - Idempotent: safe to run repeatedly; verify-before-and-after.
    - Logs: <SystemDrive>\IntuneLogs\Restart-SystemService\Restart-SystemService-Remediation.txt
#>
...
function Test-RemediationPrerequisites {
    try {
        # The placeholder must be replaced before any service can be restarted.
        if ([string]::IsNullOrWhiteSpace($servicename) -or $servicename -eq 'ServiceName') {
            throw "Template placeholder is still in use. Replace ServiceName before production use."
        }

        # The configured service must exist before Restart-Service touches it.
        $service = Get-Service -Name $servicename -ErrorAction SilentlyContinue
        if (-not $service) {
            throw "Configured service was not found: $servicename"
        }
...
function Test-FixApplied {
    try {
        # Re-check the same object type the detector read. Return $true only when
        # the observed state now matches the desired state (Running).
        $service = Get-Service -Name $servicename -ErrorAction SilentlyContinue
        if (-not $service) {
            Write-RemediationLog "Verification could not find the configured service: $servicename" -Level 'Error'
            return $false
        }
        return ($service.Status -eq 'Running')
    }
    catch {
        Write-RemediationLog "Verification could not read the service state: $($_.Exception.Message)" -Level 'Error'
        return $false
    }
}
...
try {
    ...
    # Preserved legacy outcome: an unreplaced template placeholder is a failed
    ...
    if ([string]::IsNullOrWhiteSpace($servicename) -or $servicename -eq 'ServiceName') {
        ...
        Finish-Script -ExitCode 1 -Message 'Template placeholder is still in use. Replace ServiceName before production use.' -Level 'ERROR'
    }

    # Preserved legacy outcome: a missing configured service is a failed
    # remediation, not a crash.
    $serviceInfo = Get-Service -Name $servicename -ErrorAction SilentlyContinue
    if (-not $serviceInfo) {
        $script:RemediationResult.Status = "Failed"
        Write-Output ($remediationResult | ConvertTo-Json -Depth 6 -Compress)
        Finish-Script -ExitCode 1 -Message "Configured service was not found: $servicename" -Level 'ERROR'
    }
    ...
    $targetCount++
    $fixApplied = Invoke-FixTarget -TargetName "Restart service: $servicename" -Fix {
        Write-RemediationLog "Restarting configured service: $servicename" -Level 'Info'

        Restart-Service -Name $servicename -Force -ErrorAction Stop

        Write-RemediationLog "Restart-Service completed for: $servicename" -Level 'Info'
    }
    ...
    if ($verificationPassed) {
        ...
        Finish-Script -ExitCode 0 -Message "Service restart completed successfully. Current state: Running" -Level 'SUCCESS'
    }
    else {
        $script:RemediationResult.Status = "Failed"
        Write-Output "Remediation finished but verification failed"
        Write-Output ($remediationResult | ConvertTo-Json -Depth 6 -Compress)
        Finish-Script -ExitCode 1 -Message "Post-remediation verification failed - the service did not reach the running state" -Level 'ERROR'
    }
}
catch {
