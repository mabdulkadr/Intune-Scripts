...
$ScriptName   = 'Clear-DnsClientCacheImmediate--Detect.ps1'
$SolutionName = 'Clear-DnsClientCacheImmediate'
$ScriptMode   = 'Detection'

$SystemDrive = if ($env:SystemDrive) { $env:SystemDrive.TrimEnd('\') } else { [System.IO.Path]::GetPathRoot($env:SystemRoot).TrimEnd('\') }
$LogRoot     = Join-Path $SystemDrive "IntuneLogs\$SolutionName"
$LogFile     = Join-Path $LogRoot 'Clear-DnsClientCacheImmediate--Detect.txt'
$BannerLine  = '=' * 78
...
try {
    Initialize-Log
    Write-Banner
    Finish-Script -Message 'Non-Compliant: DNS cache flush is always required.' -ExitCode 1 -Level 'WARNING'
}
catch {
